...
export function PaymentPage() {
    ...
    const handlePayment = (amount, period) => {
    ...
    return (<div className="w-full min-h-screen p-8 space-y-6">
      ...
      {/* Subscription Plans */}
      {!paymentStatus?.hasActivePayment && (<div className="p-6 rounded-2xl bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-gray-200/50 dark:border-gray-800/50 shadow-lg">
          <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {plans.map((plan) => (<div key={plan.name} className={cn('p-6 rounded-xl border-2 transition-all', plan.popular
                ...
                <button onClick={() => handlePayment(plan.price, plan.period)} disabled={createPaymentMutation.isPending} className={cn('w-full py-3 px-4 rounded-lg font-semibold transition-colors', plan.popular
                    ? 'bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))]'
                    : 'bg-gray-900 dark:bg-gray-700 hover:bg-gray-800 dark:hover:bg-gray-600 text-white', createPaymentMutation.isPending && 'opacity-50 cursor-not-allowed')}>
        ...
        {payments && payments.length > 0 ? (<div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {payments.map((payment) => (<tr key={payment._id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
