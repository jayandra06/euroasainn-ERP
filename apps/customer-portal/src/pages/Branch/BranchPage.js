    ...
    return (<div className="w-full space-y-6">
      ...
      {isLoading ? (<div className="bg-[hsl(var(--card))] rounded-lg border border-[hsl(var(--border))] p-12 text-center">
          ...
          <table className="w-full">
            ...
            <tbody>
              {businessUnits && businessUnits.length > 0 ? (businessUnits.map((unit) => {
                const metadata = unit.metadata || {};
                const representative = employees?.find((e) => e._id === (metadata.representativeId || unit.representativeId));
                return (<tr key={unit._id} className="border-b border-gray-200 dark:border-gray-700">
                      <td className="px-4 py-3 text-sm text-[hsl(var(--foreground))] font-medium">
                        <button onClick={() => navigate(`/dashboard/branch/${unit._id}`)} className="text-[hsl(var(--primary))] hover:underline cursor-pointer">
                          {unit.name}
                        </button>
                      </td>
                      ...
                      <td className="px-4 py-3 text-sm">
                        <button onClick={() => navigate(`/dashboard/branch/${unit._id}`)} className="text-[hsl(var(--primary))] hover:underline">
                          View Profile
                        </button>
                      </td>
                    </tr>);
      ...
      {/* Add Branch Modal */}
      <Modal isOpen={showAddModal} onClose={handleClose} title="Add Branch" size="medium">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Enter BU Name" required className="w-full px-4 py-2.5 border-2 rounded-lg bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-transparent transition-all border-[hsl(var(--border))]"/>
          </div>
          ...
          <div>
            ...
            <textarea value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} placeholder="Enter BU Address" rows={3} className="w-full px-4 py-2.5 border-2 rounded-lg bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-transparent transition-all border-[hsl(var(--border))] resize-none"/>
          </div>
          ...
          <div>
            ...
            <input type="tel" value={formData.phoneNumber} onChange={(e) => setFormData({ ...formData, phoneNumber: e.target.value })} placeholder="Enter BU Phone Number" className="w-full px-4 py-2.5 border-2 rounded-lg bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-transparent transition-all border-[hsl(var(--border))]"/>
          </div>
          ...
          <div>
            ...
            <select value={formData.representativeId} onChange={(e) => setFormData({ ...formData, representativeId: e.target.value })} className="w-full px-4 py-2.5 border-2 rounded-lg bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-transparent transition-all border-[hsl(var(--border))]">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-[hsl(var(--border))]">
            <button type="button" onClick={handleClose} disabled={createMutation.isPending} className="px-4 py-2 text-[hsl(var(--foreground))] bg-[hsl(var(--secondary))] hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg font-semibold transition-colors disabled:opacity-50">
              Cancel
            </button>
            <button type="submit" disabled={createMutation.isPending} className="px-4 py-2 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-white rounded-lg font-semibold transition-colors disabled:opacity-50 shadow-lg hover:shadow-xl">
