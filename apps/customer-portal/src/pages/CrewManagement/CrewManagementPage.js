...
import { MdEdit, MdDelete, MdPersonAdd, MdSearch, MdSave, MdCancel } from 'react-icons/md';
...
export function CrewManagementPage() {
    ...
    const deleteEmployeeMutation = useMutation({
        ...
        onMutate: async (employeeId) => {
            ...
            queryClient.setQueryData(['employees'], (old = []) => old.filter((emp) => emp._id !== employeeId));
            return { previousEmployees };
        },
        onSuccess: () => {
    ...
    const columns = [
        {
            key: 'employeeId',
            header: 'Employee ID',
            render: (employee) => (<span className="font-mono text-sm">{employee._id.slice(-8).toUpperCase()}</span>),
        },
        {
            key: 'name',
            header: 'Name',
            render: (employee) => (<span className="font-medium">{`${employee.firstName} ${employee.lastName}`}</span>),
        },
        {
            key: 'email',
            header: 'Email',
        ...
        },
        {
            ...
            render: (employee) => {
                ...
                if (role) {
                    return (<span className="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
              {role.name}
            </span>);
                }
                ...
                return (<span className="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400">
        ...
        },
        {
          ...
          <button onClick={() => handleEditEmployee(employee)} className="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Edit">
            <MdEdit className="w-4 h-4"/>
          </button>
          <button onClick={() => handleDeleteEmployee(employee._id)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Delete">
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 px-6 py-2.5 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))] rounded-lg font-medium transition-colors shadow-sm">
      ...
      <div className="relative">
        ...
        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search employees by name, email, phone, type, access level, or business unit..." className="w-full pl-10 pr-4 py-2.5 border border-[hsl(var(--border))] rounded-lg bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))]"/>
        </div>

      {/* Employees Table */}
      {employeesLoading ? (<div className="p-16 text-center rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]">
          <p className="text-lg font-semibold text-[hsl(var(--muted-foreground))]">Loading employees...</p>
        </div>) : (<DataTable columns={columns} data={filteredEmployees} emptyMessage={searchQuery
      ...
      {showAddModal && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmitEmployee} className="space-y-4">
              ...
              <input type="text" value={employeeForm.firstName} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, firstName: e.target.value }))} placeholder="First Name" required className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"/>
              <input type="text" value={employeeForm.lastName} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, lastName: e.target.value }))} placeholder="Last Name" required className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              ...
              <input type="email" value={employeeForm.email} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, email: e.target.value }))} placeholder="Enter email address" required className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"/>
              </div>
              <div>
                <SearchableSelect options={countryOptions} value={employeeForm.country} onChange={(selectedCountry) => {
            ...
            }} placeholder="Search and select country..." label="Country *"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Phone *
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <select value={employeeForm.phoneCountryCode} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, phoneCountryCode: e.target.value }))} className="w-full sm:w-32 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                    {countryCodeOptions.map((option) => (<option key={option.value} value={option.value}>
                        {option.label}
                      </option>))}
                  </select>
              <input type="tel" value={employeeForm.phone} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, phone: e.target.value }))} placeholder="Enter phone number" className="flex-1 min-w-0 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              ...
              <select value={employeeForm.role} onChange={(e) => {
            ...
            }} disabled={rolesLoading} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
                  <option value="">
                    {rolesLoading ? 'Loading roles from Customer Portal...' : 'Select Role'}
                  </option>
                  {roles.length > 0 ? (roles.map((role) => (<option key={role._id} value={role._id}>
                        {role.name}
                      </option>))) : !rolesLoading ? (<option value="" disabled>
                      No roles available
                    </option>) : null}
                </select>
                {!rolesLoading && roles.length === 0 && (<p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    No roles found. Create roles in Customer Portal Role Management.
                  </p>)}
              </div>

              {/* Auto-fetched Salary Template Display */}
              {employeeForm.role && (<div className="mt-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-sm font-semibold text-gray-900 dark:text-white">
                      Salary Template {isEditingSalary ? '(Editing)' : '(Auto-loaded from Payroll Management)'}
                    </h4>
                    <div className="flex items-center gap-2">
                      {payrollLoading && (<span className="text-xs text-gray-500 dark:text-gray-400">Loading...</span>)}
                      {payrollTemplate && !payrollLoading && (<>
                          {!isEditingSalary ? (<button type="button" onClick={() => {
                  ...
                  {payrollTemplate ? (<div className="space-y-2 text-sm">
                      {(() => {
                        ...
                        return (<>
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                ...
                                {isEditingSalary ? (<input type="number" value={currentBase} onChange={(e) => {
                                    const value = parseFloat(e.target.value) || 0;
                                    setSalaryOverride((prev) => ({ ...prev, base: value }));
                                }} className="ml-2 w-32 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" min="0" step="0.01"/>) : (<span className="ml-2 font-semibold text-gray-900 dark:text-white">
                              ...
                              <div>
                                ...
                                {isEditingSalary ? (<input type="number" value={currentIncentives} onChange={(e) => {
                                    const value = parseFloat(e.target.value) || 0;
                            ...
                            <div className="grid grid-cols-3 gap-4 pt-2 border-t border-gray-200 dark:border-gray-600">
                              <div>
                                ...
                                {isEditingSalary ? (<input type="number" value={currentHraPercent} onChange={(e) => {
                                    const value = parseFloat(e.target.value) || 0;
                                    setSalaryOverride((prev) => ({ ...prev, hraPercent: value }));
                                }} className="ml-2 w-24 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm" min="0" step="0.01"/>) : (<span className="ml-2 font-semibold text-gray-900 dark:text-white">
                                    {currentHraPercent.toFixed(2)}%
                                  </span>)}
                              </div>
                              <div>
                                <span className="text-gray-600 dark:text-gray-400">TA:</span>
                                {isEditingSalary ? (<input type="number" value={currentTaPercent} onChange={(e) => {
                                    const value = parseFloat(e.target.value) || 0;
                              ...
                              <div>
                                ...
                                {isEditingSalary ? (<input type="number" value={currentDaPercent} onChange={(e) => {
                                    const value = parseFloat(e.target.value) || 0;
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-2 border-t border-gray-200 dark:border-gray-600">
                              <div>
                                ...
                                {isEditingSalary ? (<input type="number" value={currentPfPercent} onChange={(e) => {
                                    const value = parseFloat(e.target.value) || 0;
                              ...
                              <div>
                                ...
                                {isEditingSalary ? (<input type="number" value={currentTdsPercent} onChange={(e) => {
                                    const value = parseFloat(e.target.value) || 0;
                            ...
                            {isEditingSalary && (<p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
                                * Salary override will be saved to this employee. Role default remains unchanged.
                              </p>)}
                          </>);
                    })()}
                    </div>) : !payrollLoading ? (<p className="text-xs text-gray-500 dark:text-gray-400">
              ...
              <select value={employeeForm.businessUnitId} onChange={(e) => setEmployeeForm((prev) => ({ ...prev, businessUnitId: e.target.value }))} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                  <option value="unassigned">Unassigned</option>
                  {businessUnits.map((bu) => (<option key={bu._id} value={bu._id}>
                      {bu.name}
                </option>))}
              </select>
              </div>
          ...
          <button type="submit" disabled={!isFormValid || inviteEmployeeMutation.isPending} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
      ...
      {/* Edit Employee Modal */}
      {showEditModal && editingEmployee && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <button type="submit" disabled={updateEmployeeMutation.isPending} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={newRoleName} onChange={(e) => setNewRoleName(e.target.value)} placeholder="Enter role name" className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Description (Optional)
                </label>
                <textarea value={newRoleDescription} onChange={(e) => setNewRoleDescription(e.target.value)} placeholder="Enter role description" rows={3} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"/>
              </div>
            </div>
            ...
            }} disabled={!newRoleName.trim() || createRoleMutation.isPending} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
...
//# sourceMappingURL=CrewManagementPage.js.map