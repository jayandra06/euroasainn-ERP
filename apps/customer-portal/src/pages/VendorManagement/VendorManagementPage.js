...
export function VendorManagementPage() {
    ...
    const filteredVendors = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            ...
            filtered = filtered.filter((vendor) => vendor.email?.toLowerCase().includes(query) ||
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (vendor) => {
                ...
                return (<span className={cn('px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full', config.className)}>
            {config.label}
          </span>);
            },
        },
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowInviteModal(true)} className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-medium transition-colors">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <select value={activeFilter} onChange={(e) => setActiveFilter(e.target.value)} className="px-3 py-2 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-lg text-sm text-[hsl(var(--foreground))]">
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search by vendor name, email, company, phone..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-lg text-sm text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))] focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"/>
          </div>
        </div>
      </div>
      ...
      {isLoading ? (<div className="bg-[hsl(var(--card))] rounded-lg border border-[hsl(var(--border))] p-12 text-center">
          ...
          <DataTable columns={columns} data={filteredVendors} emptyMessage="No vendors found. Invite vendors to get started."/>
        </div>)}

      {/* Invite Vendor Modal */}
      <Modal isOpen={showInviteModal} onClose={handleClose} title="Invite New Vendor" size="medium">
        <VendorInviteForm onSuccess={handleSuccess} onCancel={handleClose}/>
      </Modal>
    </div>);
}
//# sourceMappingURL=VendorManagementPage.js.map