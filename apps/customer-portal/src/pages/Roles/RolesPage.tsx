...
export function RolesPage() {
  ...
  useEffect(() => {
    if (user?.organizationId) {
      ...
      if (!params.get("portalType")) {
        params.set("portalType", "customer");
      }
  ...
  const fetchPermissions = async () => {
    try {
      const res = await authenticatedFetch(`/api/v1/permissions?portalType=customer`);
  ...
  // -----------------------------------------
  // LOAD ROLES FROM BACKEND (WITH TOKEN)
  // -----------------------------------------
  const fetchRoles = async () => {
    ...
    try {
      ...
      urlParams.set("portalType", "customer");
      ...
      const res = await authenticatedFetch(`/api/v1/roles?${params.toString()}`);

      const json = await res.json();
      setLoading(false);
      if (!json.success) {
        console.error("Failed to fetch roles:", json.error);
        return;
      }
      ...
      const mapped = json.data.map((r: any) => ({
        ...
        permissions: permissionsList.reduce((acc: any, p: any) => {
          acc[p.key] = r.permissions?.includes(p.key) ?? false;
          return acc;
        }, {}),
      }));
  ...
  // -----------------------------------------
  // DELETE ROLE (WITH TOKEN)
  // -----------------------------------------
  const deleteRole = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this role?")) return;

    try {
      const res = await authenticatedFetch(`/api/v1/roles/${id}`, {
        method: "DELETE",
      });
  ...
  // -----------------------------------------
  // EDIT ROLE - SAVE CHANGES (WITH TOKEN)
  // -----------------------------------------
  const saveEdit = async () => {
    ...
    try {
      const res = await authenticatedFetch(`/api/v1/roles/${editingRole._id}`, {
        method: "PUT",
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Create and manage roles with specific permissions for Customer Portal
          </p>
        </div>
      </div>
                ...
                {permissionsList.map((perm: any) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        setPermissions({ ...permissions, [perm.key]: !permissions[perm.key] })
                      }
                    ...
                    return (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-400 capitalize">
                            {r.portal}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
