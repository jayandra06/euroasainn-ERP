...
import { MdPayment, MdReceipt, MdCreditCard, MdAccountBalance, MdSchedule, MdCheckCircle, } from 'react-icons/md';
// Mock data
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie data={invoiceSummaryData} cx="50%" cy="50%" labelLine={false} outerRadius={100} fill="#8884d8" dataKey="count" label={({ status, count, amount, percent }) => `${status}: ${count} ($${amount.toLocaleString()}, ${(percent * 100).toFixed(0)}%)`}>
              {invoiceSummaryData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
            </Pie>
            <Tooltip />
          </PieChart>
        </ResponsiveContainer>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="p-4 rounded-lg bg-[hsl(var(--secondary))]">
            ...
            <div className="space-y-3">
              {subscriptionDetails.map((sub, index) => (<div key={index} className="p-3 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))]">
                  <div className="flex items-center justify-between mb-2">
      ...
      <div className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
        ...
        <div className="mt-4 p-4 rounded-lg bg-[hsl(var(--secondary))]">
          ...
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
            <div className={`h-2 rounded-full ${currentUtilization > 80 ? 'bg-red-600' : currentUtilization > 60 ? 'bg-orange-600' : 'bg-emerald-600'}`} style={{ width: `${currentUtilization}%` }}/>
          </div>
        </div>
      </div>
    </div>);
}
//# sourceMappingURL=BillingPaymentsReports.js.map