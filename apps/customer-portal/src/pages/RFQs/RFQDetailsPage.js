...
export function RFQDetailsPage() {
    ...
    const [selectedVendorTab, setSelectedVendorTab] = useState(0);
    const [selectedVendorForFinalize, setSelectedVendorForFinalize] = useState(null);
    ...
    const { data: vendorQuotations } = useQuery({
    ...
    const hasAnyFinalizedOffer = React.useMemo(() => {
        if (!vendorQuotations || vendorQuotations.length === 0)
            return false;
        return vendorQuotations.some((q) => q.status === 'finalized');
    }, [vendorQuotations]);
    // Get the finalized offer details for display
    const finalizedOffer = React.useMemo(() => {
        if (!hasAnyFinalizedOffer)
            return null;
        return vendorQuotations?.find((q) => q.status === 'finalized');
    ...
    if (isLoading) {
        return (<div className="w-full p-8 min-h-screen flex items-center justify-center bg-[hsl(var(--background))]">
    ...
    if (error || !rfqDetails) {
        ...
        <div className="text-center">
          ...
          <button onClick={() => navigate('/rfqs')} className="px-4 py-2 bg-[hsl(var(--primary))] text-white rounded-lg hover:bg-[hsl(var(--primary))]/90">
            Back to RFQs
          </button>
        </div>
      </div>);
    ...
    return (<div className="w-full min-h-screen bg-gray-900 text-white p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/rfqs')} className="p-2 rounded-lg hover:bg-gray-800 transition-colors">
        ...
        {(vesselInfo?.name || vesselInfo?._id || rfqDetails.vesselId) && (<div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
        ...
        {((rfqDetails.recipientVendorIds && rfqDetails.recipientVendorIds.length > 0) || vendorQuotations?.some((q) => q.isAdminOffer)) && (<div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
            ...
            {hasAnyFinalizedOffer && finalizedOffer && (<div className="mb-6 p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
            ...
            <div className="flex gap-4 mb-6 border-b border-gray-700">
              ...
              {(() => {
                const adminQuotation = vendorQuotations?.find((q) => q.isAdminOffer === true);
                if (!adminQuotation)
                    return null;
                const isFinalized = adminQuotation.status === 'finalized';
                return (<button onClick={() => setSelectedVendorTab(0)} className={`px-4 py-2 font-medium transition-colors relative ${selectedVendorTab === 0
                        ? 'text-purple-400'
                        : 'text-gray-400 hover:text-gray-300'}`}>
                    ⭐ Special Offer
                    <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-semibold ${isFinalized
                    ...
                    {selectedVendorTab === 0 && (<div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-400"/>)}
                  </button>);
            })()}
              
              {/* Vendor Tabs */}
              {rfqDetails.recipientVendorIds?.map((vendor, index) => {
                const quotation = vendorQuotations?.find((q) => !q.isAdminOffer && (q.vendorOrganizationId?._id === vendor._id || q.vendorOrganizationId?._id?.toString() === vendor._id?.toString()));
                const hasQuotation = !!quotation;
                const isFinalized = quotation?.status === 'finalized';
                const tabIndex = (vendorQuotations?.some((q) => q.isAdminOffer) ? 1 : 0) + index;
                return (<button key={vendor._id?.toString() || index} onClick={() => setSelectedVendorTab(tabIndex)} className={`px-4 py-2 font-medium transition-colors relative ${selectedVendorTab === tabIndex
                        ? 'text-blue-400'
                        : 'text-gray-400 hover:text-gray-300'}`}>
                    {vendor.name || `Vendor ${index + 1}`}
                    {hasQuotation && (<span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-semibold ${isFinalized
                            ? 'bg-green-500/20 text-green-400'
                    ...
                    {!hasQuotation && (<span className="ml-2 px-2 py-0.5 bg-gray-600/50 text-gray-400 rounded-full text-xs font-semibold">
                        Waiting
                      </span>)}
                    {selectedVendorTab === tabIndex && (<div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-400"/>)}
                  </button>);
            })}
            </div>
            ...
            {(() => {
                ...
                if (selectedVendorTab === 0 && hasAdminOffer) {
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <span className={`px-3 py-1 rounded-full text-sm font-semibold ${isFinalized
                            ? 'bg-green-500/20 text-green-400'
                            : adminQuotation.status === 'submitted'
                                ? 'bg-blue-500/20 text-blue-400'
                                : 'bg-yellow-500/20 text-yellow-400'}`}>
                        ...
                        {adminQuotation.submittedAt && (<p className="text-sm text-purple-300 mt-1">
                    ...
                    {/* Special Offer Items */}
                    {adminQuotation.items && adminQuotation.items.length > 0 ? (<div>
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full border-collapse">
                            ...
                            <tbody>
                              {adminQuotation.items.map((item, idx) => (<tr key={idx} className="border-b border-purple-500/20 hover:bg-purple-900/20">
                    ...
                    {adminQuotation.terms && (<div>
                        ...
                        <div className="grid grid-cols-2 gap-6">
                          ...
                          {adminQuotation.terms.paymentTerm && (<div>
                          ...
                          {adminQuotation.terms.insuranceTerm && (<div>
                          ...
                          {adminQuotation.terms.taxTerm && (<div>
                          ...
                          {adminQuotation.terms.transportTerm && (<div>
                          ...
                          {adminQuotation.terms.deliveryTerm && (<div>
                          ...
                          {adminQuotation.terms.packingTerm && (<div>
                          ...
                          {adminQuotation.terms.currency && (<div>
                    ...
                    {!isFinalized && (<div className="mt-6 pt-6 border-t border-purple-500/30">
                        {hasAnyFinalizedOffer && (<div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                        ...
                        <button onClick={() => {
                            ...
                            }} disabled={finalizeOfferMutation.isPending || hasAnyFinalizedOffer} className="w-full px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                          {finalizeOfferMutation.isPending ? 'Finalizing...' : hasAnyFinalizedOffer ? 'Offer Already Finalized' : 'Finalize Special Offer'}
                        </button>
                      </div>)}

                    {isFinalized && (<div className="mt-6 p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
                ...
                const selectedVendor = rfqDetails.recipientVendorIds?.[vendorIndex];
                if (!selectedVendor)
                    return null;
                const quotation = vendorQuotations?.find((q) => !q.isAdminOffer && (q.vendorOrganizationId?._id === selectedVendor._id || q.vendorOrganizationId?._id?.toString() === selectedVendor._id?.toString()));
                const hasQuotation = !!quotation;
                const isFinalized = quotation?.status === 'finalized';
                ...
                return (<div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      {hasQuotation ? (<>
                          ...
                          {quotation.submittedAt && (<p className="text-sm text-gray-400 mt-1">
                  ...
                  {hasQuotation && quotation.items && quotation.items.length > 0 ? (<div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full border-collapse">
                          ...
                          <tbody>
                            {quotation.items.map((item, idx) => (<tr key={idx} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                                <td className="p-3 text-white">{idx + 1}</td>
                                <td className="p-3 text-white">{item.description || '-'}</td>
                                <td className="p-3 text-white">{item.requiredQty || '-'}</td>
                                <td className="p-3 text-white">${item.quotedPrice?.toFixed(2) || '0.00'}</td>
                    ...
                    </div>) : hasQuotation ? (<div className="text-center py-8">
                  ...
                  {hasQuotation && quotation.terms && (<div>
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        ...
                        {quotation.terms.paymentTerm && (<div>
                        ...
                        {quotation.terms.insuranceTerm && (<div>
                        ...
                        {quotation.terms.transportTerm && (<div>
                        ...
                        {quotation.terms.deliveryTerm && (<div>
                        ...
                        {quotation.terms.packingTerm && (<div>
                  ...
                  {hasQuotation && !isFinalized && (<div className="mt-6 pt-6 border-t border-gray-700">
                      {hasAnyFinalizedOffer && (<div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                          <p className="text-sm text-yellow-400">
                            ⚠️ Another offer has already been finalized. You cannot finalize additional offers.
                          </p>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        }} disabled={finalizeOfferMutation.isPending || isSelected || hasAnyFinalizedOffer} className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  {finalizeOfferMutation.isError && (<div className="mt-4 p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
                      <p className="text-sm text-red-400">
                        Error: {finalizeOfferMutation.error instanceof Error ? finalizeOfferMutation.error.message : 'Failed to finalize offer'}
                      </p>
                    </div>)}
                  
                  {finalizeOfferMutation.isSuccess && (<div className="mt-4 p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
        ...
        {bankingDetails && bankingDetails.length > 0 && (<div className="bg-gray-800 rounded-lg p-6 border border-gray-700 mt-6">
            <h2 className="text-xl font-semibold mb-6 pb-2 border-b border-gray-700">Banking Details</h2>
            <div className="space-y-6">
              {bankingDetails.map((banking, index) => (<div key={index} className="bg-gray-700/50 rounded-lg p-6 border border-gray-600">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-white">
                      {banking.organizationId?.name || 'Vendor'} - Quotation #{banking.quotationId?.quotationNumber || 'N/A'}
                    </h3>
                    {banking.submittedAt && (<span className="text-sm text-gray-400">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    {banking.bankAddress && (<div className="col-span-2">
                    ...
                    {banking.currency && (<div>
                        <label className="text-sm font-medium text-gray-400 block mb-1">Currency</label>
                        <p className="text-white">{banking.currency}</p>
                      </div>)}
                    {banking.notes && (<div className="col-span-2">
                        <label className="text-sm font-medium text-gray-400 block mb-1">Notes</label>
                        <p className="text-white">{banking.notes}</p>
                      </div>)}
                  </div>
                  {banking.documents && banking.documents.length > 0 && (<div className="mt-4 pt-4 border-t border-gray-600">
                      <label className="text-sm font-medium text-gray-400 block mb-2">Uploaded Documents</label>
                      <div className="space-y-2">
                        {banking.documents.map((doc, docIndex) => (<div key={docIndex} className="flex items-center gap-2 p-2 bg-gray-600/50 rounded">
                            <span className="text-sm text-white">{doc.fileName}</span>
                            {doc.fileUrl && (<a href={doc.fileUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 text-sm">
                                View
                              </a>)}
                          </div>))}
                      </div>
              ...
              {bankingDetails.map((banking, index) => {
                const quotationId = banking.quotationId?._id || banking.quotationId;
                const existingPaymentProof = paymentProofs?.find((proof) => proof.quotationId?._id?.toString() === quotationId?.toString() ||
                    proof.quotationId?.toString() === quotationId?.toString());
                return (<div key={`payment-${index}`}>
                    <PaymentSubmissionForm quotationId={quotationId} bankingDetails={banking} existingPaymentProof={existingPaymentProof}/>
                    {/* Shipping Selection - Show after payment is approved */}
                    {existingPaymentProof?.status === 'approved' && (<ShippingSelectionForm quotationId={quotationId} paymentProof={existingPaymentProof}/>)}
                  </div>);
            })}
            </div>
          </div>)}
        ...
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          ...
          {rfqItems && rfqItems.length > 0 ? (<div className="overflow-x-auto mt-4">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b border-gray-700">
                    <th className="text-left p-3 text-sm font-semibold text-gray-400">#</th>
                ...
                <tbody>
                  {rfqItems.map((item, idx) => (<tr key={idx} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                      <td className="p-3 text-white">{idx + 1}</td>
              ...
              {process.env.NODE_ENV === 'development' && rfqDetails?.metadata && (<details className="mt-4">
...
function PaymentSubmissionForm({ quotationId, bankingDetails, existingPaymentProof }) {
    const queryClient = useQueryClient();
    ...
    if (existingPaymentProof) {
        return (<div className="mt-6 p-6 bg-green-500/10 border border-green-500/30 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {existingPaymentProof.paymentMethod && (<div>
          ...
          {existingPaymentProof.transactionReference && (<div>
        ...
        {existingPaymentProof.proofDocuments && existingPaymentProof.proofDocuments.length > 0 && (<div className="mt-4 pt-4 border-t border-green-500/30">
            <label className="text-sm font-medium text-gray-400 block mb-2">Payment Proof Documents</label>
            <div className="space-y-2">
              {existingPaymentProof.proofDocuments.map((doc, docIndex) => (<div key={docIndex} className="flex items-center gap-2 p-2 bg-gray-700/50 rounded">
                  <span className="text-sm text-white">{doc.fileName}</span>
                  {doc.fileUrl && (<a href={doc.fileUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 text-sm">
                      View
                    </a>)}
        ...
        {existingPaymentProof.submittedAt && (<p className="text-sm text-gray-400 mt-4">
            Submitted: {new Date(existingPaymentProof.submittedAt).toLocaleString()}
          </p>)}
      </div>);
    }
    return (<div className="mt-6 p-6 bg-blue-500/10 border border-blue-500/30 rounded-lg">
      <div className="flex items-center justify-between mb-4">
      ...
      {showForm && (<form onSubmit={handleSubmit} className="space-y-6 mt-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" step="0.01" required min="0" value={paymentData.paymentAmount} onChange={(e) => setPaymentData({ ...paymentData, paymentAmount: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white" placeholder="0.00"/>
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Currency</label>
              <select value={paymentData.currency} onChange={(e) => setPaymentData({ ...paymentData, currency: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white">
            ...
            <div>
              ...
              <input type="date" required value={paymentData.paymentDate} onChange={(e) => setPaymentData({ ...paymentData, paymentDate: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white"/>
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Payment Method</label>
              <select value={paymentData.paymentMethod} onChange={(e) => setPaymentData({ ...paymentData, paymentMethod: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white">
            ...
            <div>
              ...
              <input type="text" value={paymentData.transactionReference} onChange={(e) => setPaymentData({ ...paymentData, transactionReference: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white" placeholder="Transaction ID or Reference Number"/>
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium mb-2">Notes</label>
              <textarea value={paymentData.notes} onChange={(e) => setPaymentData({ ...paymentData, notes: e.target.value })} rows={3} className="w-full p-2 border rounded-lg bg-gray-700 text-white" placeholder="Any additional notes about the payment..."/>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-gray-500 rounded-lg p-4">
              <input type="file" multiple accept="image/*,.pdf" onChange={handleFileUpload} className="hidden" id="payment-proof-upload"/>
              <label htmlFor="payment-proof-upload" className="cursor-pointer flex items-center gap-2 text-blue-400 hover:text-blue-300">
                <MdUpload className="w-5 h-5"/>
                <span>Click to upload payment proof files</span>
              </label>
              {uploadedFiles.length > 0 && (<div className="mt-4 space-y-2">
                  {uploadedFiles.map((file, index) => (<div key={index} className="flex items-center justify-between p-2 bg-gray-700 rounded">
          ...
          <button type="submit" disabled={submitPaymentMutation.isPending} className="w-full px-6 py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
...
function ShippingSelectionForm({ quotationId, paymentProof }) {
    const queryClient = useQueryClient();
    const [selectedShipping, setSelectedShipping] = useState(paymentProof.shippingOption || null);
    ...
    const handleSelfShippingSubmit = (e) => {
        e.preventDefault();
    ...
    if (paymentProof.shippingOption) {
        ...
        <div className="p-4 bg-gray-700/50 rounded-lg">
          ...
          {paymentProof.shippingOption === 'self' ? (<div className="space-y-2 mt-3">
              <p className="text-sm text-gray-300">
                You will arrange your own shipping and logistics.
              </p>
              {paymentProof.awbTrackingNumber && (<div className="mt-3 pt-3 border-t border-gray-600">
            ...
            </div>) : paymentProof.vendorAWBTrackingNumber ? (
            ...
            <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                ...
                {paymentProof.vendorShippingSubmittedAt && (<p className="text-xs text-gray-400 mt-3 pt-2 border-t border-blue-500/30">
          ...
          {paymentProof.shippingSelectedAt && (<p className="text-xs text-gray-400 mt-3">
              Selected: {new Date(paymentProof.shippingSelectedAt).toLocaleString()}
            </p>)}
        </div>
      </div>);
    }
    return (<div className="mt-6 p-6 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
      <div className="flex items-center gap-3 mb-4">
      ...
      {!showSelfShippingForm ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button onClick={() => handleShippingSelect('self')} disabled={selectShippingMutation.isPending} className="p-6 bg-gray-700 hover:bg-gray-600 border-2 border-gray-600 hover:border-blue-500 rounded-lg transition-all text-left disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <button onClick={() => handleShippingSelect('vendor-managed')} disabled={selectShippingMutation.isPending} className="p-6 bg-gray-700 hover:bg-gray-600 border-2 border-gray-600 hover:border-blue-500 rounded-lg transition-all text-left disabled:opacity-50 disabled:cursor-not-allowed">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-2xl">🚢</span>
        ...
        </div>) : (<div className="mt-4 p-4 bg-gray-800 rounded-lg border border-gray-700">
          <div className="flex items-center justify-between mb-4">
          ...
          <form onSubmit={handleSelfShippingSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" required value={selfShippingData.awbTrackingNumber} onChange={(e) => setSelfShippingData({ ...selfShippingData, awbTrackingNumber: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white" placeholder="Enter AWB tracking number"/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">
                Contact Name <span className="text-red-500">*</span>
              </label>
              <input type="text" required value={selfShippingData.shippingContactName} onChange={(e) => setSelfShippingData({ ...selfShippingData, shippingContactName: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white" placeholder="Enter contact name"/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">
                Contact Email <span className="text-red-500">*</span>
              </label>
              <input type="email" required value={selfShippingData.shippingContactEmail} onChange={(e) => setSelfShippingData({ ...selfShippingData, shippingContactEmail: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white" placeholder="Enter email address"/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">
                Contact Phone Number <span className="text-red-500">*</span>
              </label>
              <input type="tel" required value={selfShippingData.shippingContactPhone} onChange={(e) => setSelfShippingData({ ...selfShippingData, shippingContactPhone: e.target.value })} className="w-full p-2 border rounded-lg bg-gray-700 text-white" placeholder="Enter phone number"/>
            </div>
            <button type="submit" disabled={selectShippingMutation.isPending} className="w-full px-6 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              {selectShippingMutation.isPending ? 'Submitting...' : 'Submit Shipping Details'}
            </button>
          </form>
        </div>)}

      {selectedShipping === 'vendor-managed' && !showSelfShippingForm && paymentProof?.vendorAWBTrackingNumber && (<div className="mt-4 p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
          <h4 className="text-blue-400 font-bold mb-3 pb-2 border-b border-blue-500/30">
            📦 Shipping Details from Vendor
          </h4>
          <div className="space-y-2 text-sm">
      ...
      {selectedShipping === 'vendor-managed' && !showSelfShippingForm && !paymentProof?.vendorAWBTrackingNumber && (<div className="mt-4 p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
          <p className="text-green-400 font-semibold text-center">
            😊 We will take care of the rest, chillax! Waiting for vendor to submit shipping details...
          </p>
        </div>)}

      {selectShippingMutation.isPending && (<p className="text-sm text-yellow-300 mt-4 text-center">Processing...</p>)}
    </div>);
}
