...
export function TemplateLayout() {
    ...
    return (<div className="flex h-screen w-full bg-[hsl(var(--background))] text-[hsl(var(--foreground))] overflow-hidden">
      <Sidebar collapsed={sidebarCollapsed} onToggle={(collapsed) => setSidebarCollapsed(collapsed)}/>
      <div className={`flex-1 flex flex-col h-full transition-all duration-300 ease-in-out bg-[hsl(var(--background))] ${sidebarCollapsed ? 'ml-20' : 'ml-72'}`}>
