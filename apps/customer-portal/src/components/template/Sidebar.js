...
import { MdDashboard, MdLocalShipping, MdPeople, MdChevronLeft, MdChevronRight, MdRocketLaunch, MdLogout, MdDirectionsBoat, MdAccountBalance, MdTrendingUp, MdRoute, MdVerified, MdWarning, MdEco, MdLocationOn, MdManageAccounts, MdBusiness, MdKeyboardArrowDown, MdKeyboardArrowUp, MdHelp, MdBarChart, MdVpnKey, MdPayment, MdGroups, MdAccountBalanceWallet, } from 'react-icons/md';
...
export function Sidebar({ collapsed, onToggle }) {
    ...
    const isSubmenuExpanded = (label) => expandedMenus.has(label);
      ...
      <nav className={cn('flex-1 overflow-y-auto overflow-x-hidden py-4 space-y-1', collapsed ? 'px-2' : 'px-3')}>
        {navItems.map((item) => {
            ...
            if (hasSubmenu) {
                ...
                <button onClick={() => !collapsed && toggleSubmenu(item.label)} onMouseEnter={(e) => handleMouseEnter(e, item.label)} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className={cn('group relative w-full flex items-center gap-3 py-2.5 rounded-lg transition-all duration-200', collapsed ? 'justify-center px-2' : 'px-3', isSubmenuActive
                  ...
                  {!collapsed && (<>
                      ...
                      {isExpanded ? (<MdKeyboardArrowUp className="w-4 h-4 flex-shrink-0"/>) : (<MdKeyboardArrowDown className="w-4 h-4 flex-shrink-0"/>)}
                    </>)}
                </button>
                {!collapsed && isExpanded && (<div className="ml-4 mt-1 space-y-1 border-l border-[hsl(var(--border))] pl-4">
                    {item.submenu.map((subItem) => {
                            const isSubActive = location.pathname === subItem.path;
                            return (<NavLink key={subItem.path} to={subItem.path} className={cn('group relative flex items-center gap-2 py-2 rounded-lg transition-all duration-200 text-sm', isSubActive
                                    ? 'bg-[hsl(var(--primary))]/10 text-[hsl(var(--primary))] font-semibold'
            ...
            return (<NavLink key={item.path || item.label} to={item.path || '#'} onMouseEnter={(e) => handleMouseEnter(e, item.label)} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className={cn('group relative flex items-center gap-3 py-2.5 rounded-lg transition-all duration-200', collapsed ? 'justify-center px-2' : 'px-3', isActive
                    ? 'bg-[hsl(var(--primary))]/10 text-[hsl(var(--primary))] font-semibold'
                    : 'text-[hsl(var(--foreground))] hover:bg-[hsl(var(--muted))] hover:text-[hsl(var(--foreground))]')}>
