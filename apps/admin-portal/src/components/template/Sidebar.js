...
import { MdDashboard, MdBusinessCenter, MdVpnKey, MdChevronLeft, MdChevronRight, MdRocketLaunch, MdLogout, MdPeople, MdSettings, MdBarChart, MdAssignment, MdRequestQuote, MdInventory, MdStore, MdCategory, MdModelTraining, MdExpandMore, MdExpandLess, MdSupport, } from 'react-icons/md';
...
export function Sidebar({ collapsed, onToggle }) {
    ...
    return (<aside className={cn('fixed left-0 top-0 z-50 h-screen bg-[hsl(var(--background))] border-r border-[hsl(var(--border))] transition-all duration-300 flex flex-col overflow-x-hidden overflow-y-auto', collapsed ? 'w-20' : 'w-72')}>
      ...
      <div className={cn('flex items-center justify-between border-b border-[hsl(var(--border))] bg-[hsl(var(--background))] overflow-hidden', collapsed ? 'px-2 py-4' : 'px-6 py-5')}>
        ...
        <button onClick={() => onToggle(!collapsed)} className={cn('rounded-lg hover:bg-[hsl(var(--muted))] transition-colors text-[hsl(var(--muted-foreground))] flex-shrink-0', collapsed ? 'p-1' : 'p-1.5')}>
        ...
        {navItems.map((item) => {
            ...
            if (hasChildren) {
                ...
                <button onClick={() => {
                    ...
                    }} onMouseEnter={(e) => handleMouseEnter(e, item.label)} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className={cn('group relative flex items-center gap-3 py-2.5 rounded-lg transition-all duration-200 w-full text-left', collapsed ? 'justify-center px-2' : 'px-3', hasActiveChild
                ...
                {/* Child Items */}
                {!collapsed && isExpanded && item.children && (<div className="ml-4 mt-1 space-y-1">
                    {item.children.map((child) => {
                            const ChildIcon = child.icon;
                            const isChildActive = location.pathname === child.path;
                            return (<NavLink key={child.path} to={child.path} onMouseEnter={(e) => handleMouseEnter(e, child.label)} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className={cn('group relative flex items-center gap-3 py-2 rounded-lg transition-all duration-200 px-3', isChildActive
                                    ? 'bg-[hsl(var(--primary))]/10 text-[hsl(var(--primary))] font-semibold'
