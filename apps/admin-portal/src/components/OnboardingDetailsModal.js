...
export function OnboardingDetailsModal({ isOpen, onClose, organizationId, organizationType, organizationName, }) {
    ...
    const { data: onboardingData, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const endpoint = organizationType === 'customer'
    ...
    if (!isOpen)
        return null;
    return (<Modal isOpen={isOpen} onClose={onClose} title={`Onboarding Details - ${organizationName}`} size="large">
      {isLoading ? (<div className="p-8 text-center">
          ...
          </div>) : !onboardingData ? (<div className="p-8 text-center">
            ...
            {organizationType === 'vendor' && (<>
                ...
                {(onboardingData.brands?.length > 0 ||
                    ...
                    onboardingData.models?.length > 0) && (<div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 pb-2 border-b border-gray-200 dark:border-gray-700">
                      Product Information
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {onboardingData.brands?.length > 0 && (<div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Brands
                          </label>
                          <div className="flex flex-wrap gap-2">
                            {onboardingData.brands.map((brand, i) => (<span key={i} className="px-3 py-1 text-xs bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300 rounded-full">
                                {brand}
                              </span>))}
                          </div>
                        </div>)}
                      {onboardingData.categories?.length > 0 && (<div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Categories
                          </label>
                          <div className="flex flex-wrap gap-2">
                            {onboardingData.categories.map((category, i) => (<span key={i} className="px-3 py-1 text-xs bg-purple-100 dark:bg-purple-900/50 text-purple-800 dark:text-purple-300 rounded-full">
                                {category}
                              </span>))}
                          </div>
                        </div>)}
                      {onboardingData.models?.length > 0 && (<div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {onboardingData.models.map((model, i) => (<span key={i} className="px-3 py-1 text-xs bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300 rounded-full">
                                {model}
                              </span>))}
                          </div>
                        </div>)}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <span className={`inline-block px-3 py-1 text-xs font-semibold rounded-full ${onboardingData.status === 'approved'
            ...
            <div className="pt-6 border-t border-gray-200 dark:border-gray-700">
              {showRejectInput ? (<div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                      Rejection Reason (Optional)
                    </label>
                    <textarea value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)} placeholder="Enter reason for rejection..." rows={3} className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500"/>
                  </div>
                  <div className="flex gap-3 justify-end">
                    <button onClick={handleCancelReject} disabled={rejectMutation.isPending} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                      Cancel
                    </button>
                    <button onClick={handleReject} disabled={rejectMutation.isPending} className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                      {rejectMutation.isPending ? 'Rejecting...' : 'Confirm Reject'}
                    </button>
                  </div>
                </div>) : (<div className="flex gap-3 justify-end">
                  {canApproveReject && (<>
                      <button onClick={handleReject} disabled={rejectMutation.isPending || approveMutation.isPending} className="px-6 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        Reject
                      </button>
                      <button onClick={handleApprove} disabled={approveMutation.isPending || rejectMutation.isPending} className="px-6 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                        {approveMutation.isPending ? 'Approving...' : 'Approve'}
                      </button>
                    </>)}
                  {(isApproved || isRejected) && (<div className="text-sm text-gray-600 dark:text-gray-400 italic">
...
//# sourceMappingURL=OnboardingDetailsModal.js.map