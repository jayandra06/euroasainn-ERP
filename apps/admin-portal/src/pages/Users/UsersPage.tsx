 ...
 * Top 3 buttons ENABLED as requested
 */
...
export function UsersPage() {
  ...
  // Keeping permission checks for reference (you can re-enable later)
  ...
  const { data: usersResponse, isLoading, error: queryError } = useQuery<{
    ...
    queryKey: ['admin-users', filterActive, page, rowsPerPage, searchTerm],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (filterActive !== 'all') {
  ...
  const bulkUploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const response = await fetch(`${API_URL}/api/v1/admin/users/bulk-upload`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
        },
      ...
      if (!response.ok) {
        const errorMsg = responseData.error || responseData.message || 'Bulk upload failed';
        console.error('❌ Bulk Upload Error:', errorMsg, responseData);
    ...
    onSuccess: (data) => {
      ...
      const skippedItems = responseData?.details?.skipped || [];
      const createdItems = responseData?.details?.created || [];
      const updatedItems = responseData?.details?.updated || [];
      
      // Log detailed information
      console.log('📊 Bulk Upload Results:', {
        created: { count: created, items: createdItems },
        updated: { count: updated, items: updatedItems },
        skipped: { count: skipped, items: skippedItems },
        errors: errors,
      });
      ...
      if (errors?.length > 0) {
        ...
        let errorMessage = `Found ${errors.length} error(s): ${errorPreview.join(' | ')}`;
        if (remainingErrors > 0) {
          errorMessage += ` ... and ${remainingErrors} more. Check console (F12) for full details.`;
        } else {
        ...
        // Also show skipped items summary
        if (skippedItems.length > 0) {
          setTimeout(() => {
            const skippedMsg = skippedItems.length <= 10 
              ? `Skipped: ${skippedItems.join(', ')}` 
              : `Skipped ${skippedItems.length} items. First 10: ${skippedItems.slice(0, 10).join(', ')}. Check console (F12) for full list.`;
            toast.warning(skippedMsg);
          }, 1500);
        }
      } else if (skipped > 0) {
        // If there are skipped items but no errors, show info about skipped items
        console.warn('⚠️ Skipped items (no errors):', skippedItems);
        setTimeout(() => {
          const skippedMsg = skippedItems.length <= 10 
            ? `Skipped ${skippedItems.length} items (no changes or duplicates): ${skippedItems.join(', ')}` 
            : `Skipped ${skippedItems.length} items (no changes or duplicates). Check console (F12) for full list.`;
          toast.warning(skippedMsg);
        }, 500);
      }
    },
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {/* Top 3 buttons - Fully ENABLED */}
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => setIsSampleDataModalOpen(true)}
            className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-semibold shadow-sm transition-all bg-blue-600 text-white hover:bg-blue-700"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-semibold shadow-sm transition-all bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] hover:opacity-90"
          >
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 py-6">
              ...
              {/* Sliding Pagination - Only show if more than 1 page */}
              {totalPages > 1 && (
                <div className="flex items-center gap-2">
                  <button
