...
export function UsersPage() {
    ...
    const columns = [
        ...
        },
        {
            key: 'role',
            header: 'Role',
            render: (user) => (<span className="px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-[hsl(var(--foreground))] font-semibold dark:bg-blue-900/30">
        ...
        },
        {
            key: 'portalType',
            header: 'Portal Type',
            render: (user) => (<span className="px-3 py-1 text-xs font-semibold rounded-full bg-purple-100 text-[hsl(var(--foreground))] font-semibold dark:bg-purple-900/30">
        ...
        },
        {
            key: 'isActive',
            header: 'Status',
            render: (user) => (<span className={cn('px-3 py-1 inline-flex items-center gap-1.5 text-xs font-semibold rounded-full', user.isActive
        ...
        },
        {
            key: 'lastLogin',
            header: 'Last Login',
            render: (user) => (<div className="text-sm text-gray-600 dark:text-gray-400">
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button type="button" onClick={handleInvite} disabled={!canCreate} className={cn('relative flex items-center justify-center gap-2 px-4 py-2 rounded-xl font-semibold shadow-sm transition-all', canCreate
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {stats.map((stat, index) => {
            ...
            return (<div key={index} className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="flex items-center gap-3 flex-1 w-full sm:w-auto">
            <div className="relative flex-1 sm:max-w-md">
              ...
              <input type="text" placeholder="Search users..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all"/>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <MdFilterList className="w-5 h-5 text-gray-400"/>
            <select value={filterActive} onChange={(e) => setFilterActive(e.target.value)} className="px-4 py-2.5 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all">
      ...
      {isLoading ? (<div className="p-12 text-center rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
        ...
        </div>) : queryError ? (<div className="p-12 text-center rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/20 shadow-sm">
          ...
          <button onClick={() => window.location.reload()} className="mt-4 px-4 py-2 bg-[hsl(var(--destructive))] text-[hsl(var(--destructive-foreground))] rounded-lg hover:bg-red-700 transition-colors">
            Retry
          </button>
        </div>) : (<DataTable columns={columns} data={filteredUsers || []} onEdit={handleEdit} onDelete={handleDelete} canEdit={canUpdate} canDelete={canDelete} emptyMessage="No users found."/>)}

      {/* Modal */}
      <Modal isOpen={isModalOpen} onClose={handleClose} title={editingUser ? 'Edit User' : 'Invite User'} size="large">
