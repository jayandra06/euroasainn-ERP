...
import { MdSearch, MdFilterList, MdDownload, MdRefresh, MdBusiness, MdCheckCircle, MdCancel } from 'react-icons/md';
...
export function LoginsPage() {
    ...
    const filteredLogins = useMemo(() => {
        ...
        if (filterActive !== 'all') {
            filtered = filtered.filter(login => filterActive === 'active' ? login.isActive : !login.isActive);
        }
        // Search filter
        if (searchQuery.trim()) {
            const query = searchQuery.toLowerCase();
            filtered = filtered.filter(login => login.fullName.toLowerCase().includes(query) ||
    ...
    const columns = [
        {
            key: 'user',
            header: 'User',
            render: (login) => (<div className="flex items-center gap-3">
        ...
        },
        {
            key: 'portalType',
            header: 'Portal',
            render: (login) => (<span className={cn('px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full', login.portalType === 'customer'
        ...
        },
        {
            key: 'organization',
            header: 'Organization',
            render: (login) => (<div className="flex items-center gap-2">
        ...
        },
        {
            key: 'role',
            header: 'Role',
            render: (login) => (<span className="text-sm text-[hsl(var(--muted-foreground))] capitalize">
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl transition-colors font-semibold shadow-lg shadow-emerald-500/30">
      ...
      <div className="p-6 rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-[hsl(var(--secondary))] border border-[hsl(var(--border))] focus-within:border-emerald-500 dark:focus-within:border-emerald-400 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
            ...
            <input type="text" placeholder="Search by name, email, organization..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 bg-transparent border-none outline-none text-sm text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))]"/>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <select value={filterPortal} onChange={(e) => setFilterPortal(e.target.value)} className="px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 font-medium">
              <option value="all">All Portals</option>
              <option value="customer">Customer Portal</option>
              <option value="vendor">Vendor Portal</option>
            </select>
            <select value={filterActive} onChange={(e) => setFilterActive(e.target.value)} className="px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 font-medium">
      ...
      {isLoading ? (<div className="p-12 text-center rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
          ...
          <DataTable columns={columns} data={filteredLogins} emptyMessage="No logins found."/>
        </div>)}
    </div>);
}
//# sourceMappingURL=LoginsPage.js.map