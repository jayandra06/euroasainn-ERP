...
export function OnboardingDataPage() {
    ...
    const { data: customerOnboardings, isLoading: customerLoading, error: customerError } = useQuery({
    ...
    const { data: vendorOnboardings, isLoading: vendorLoading, error: vendorError } = useQuery({
    ...
    const filteredCustomerOnboardings = useMemo(() => {
        if (!searchQuery)
            return customerOnboardings || [];
        const query = searchQuery.toLowerCase();
        return (customerOnboardings || []).filter(item => item.companyName.toLowerCase().includes(query) ||
    ...
    const filteredVendorOnboardings = useMemo(() => {
        ...
        const query = searchQuery.toLowerCase();
        return (vendorOnboardings || []).filter(item => item.companyName.toLowerCase().includes(query) ||
            item.contactPerson.toLowerCase().includes(query) ||
            item.email.toLowerCase().includes(query));
    ...
    const filteredAllOnboardings = useMemo(() => {
        ...
        const query = searchQuery.toLowerCase();
        return allOnboardings.filter(item => item.companyName.toLowerCase().includes(query) ||
            item.contactPerson.toLowerCase().includes(query) ||
            item.email.toLowerCase().includes(query));
    }, [allOnboardings, searchQuery]);
    const handleOnboardingClick = (item, type) => {
    ...
    const customerColumns = [
        ...
        },
        {
            ...
            render: (item) => (<span className="text-[hsl(var(--muted-foreground))]">{item.contactPerson}</span>),
        },
        {
            key: 'email',
            header: 'Email',
            render: (item) => (<span className="text-[hsl(var(--muted-foreground))]">{item.email}</span>),
        },
        {
            key: 'status',
            header: 'Status',
            render: (item) => {
                ...
                return (<span className={cn('px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full ring-1', statusColors[item.status] || statusColors.pending)}>
        ...
        },
        {
            key: 'submittedAt',
            header: 'Submitted At',
            render: (item) => (<span className="text-[hsl(var(--muted-foreground))]">
        ...
        },
        {
            ...
            <button onClick={() => handleApproveCustomer(item._id)} disabled={isProcessing} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              <MdCheckCircle className="w-4 h-4"/>
              Approve
            </button>
            <button onClick={() => handleRejectCustomer(item._id)} disabled={isProcessing} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive))]/90 text-white text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    const vendorColumns = [
        ...
        },
        {
            ...
            render: (item) => {
                ...
                return (<div className="flex items-center gap-2">
            <button onClick={() => handleApproveVendor(item._id)} disabled={isProcessing} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              <MdCheckCircle className="w-4 h-4"/>
              Approve
            </button>
            <button onClick={() => handleRejectVendor(item._id)} disabled={isProcessing} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive))]/90 text-white text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              <MdCancel className="w-4 h-4"/>
              Reject
            </button>
          </div>);
    ...
    const combinedColumns = [
        {
            key: 'type',
            header: 'Type',
            render: (item) => (<span className={cn('px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full', item.type === 'customer'
        ...
        },
        {
            ...
            render: (item) => (<button onClick={(e) => {
                ...
                }} className={`font-semibold text-gray-900 dark:text-white transition-colors text-left ${item.type === 'customer'
        ...
        },
        {
            ...
            render: (item) => {
                ...
                return (<div className="flex items-center gap-2">
            <button onClick={handleApprove} disabled={isProcessing} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              <MdCheckCircle className="w-4 h-4"/>
              Approve
            </button>
            <button onClick={handleReject} disabled={isProcessing} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive))]/90 text-white text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              <MdCancel className="w-4 h-4"/>
              Reject
            </button>
          </div>);
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleRefresh} disabled={isRefreshing} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))] hover:bg-[hsl(var(--muted))] transition-colors text-sm font-medium text-[hsl(var(--foreground))] disabled:opacity-50 disabled:cursor-not-allowed">
            <MdRefresh className={cn('w-4 h-4', isRefreshing && 'animate-spin')}/>
            Refresh
          </button>
          <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))] hover:bg-[hsl(var(--muted))] transition-colors text-sm font-medium text-[hsl(var(--foreground))]">
      ...
      <div className="p-6 rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-[hsl(var(--secondary))] border border-[hsl(var(--border))] focus-within:border-[hsl(var(--primary))] focus-within:ring-2 focus-within:ring-[hsl(var(--primary))]/20 transition-all">
            ...
            <input type="text" placeholder="Search by company name, contact person, or email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 bg-transparent border-none outline-none text-sm text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))]"/>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] shadow-sm focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all duration-200 font-medium">
              <option value="all">All Types</option>
              <option value="customer">Customer Only</option>
              <option value="vendor">Vendor Only</option>
            </select>
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] shadow-sm focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all duration-200 font-medium">
      ...
      {/* Tables */}
      {isLoading ? (<div className="p-12 text-center rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
          ...
          {filterType === 'all' && (<div className="p-6 rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
              <div className="flex items-center justify-between mb-4">
                <p className="text-sm text-[hsl(var(--muted-foreground))]">
                  Showing {filteredAllOnboardings.length} onboarding{filteredAllOnboardings.length !== 1 ? 's' : ''}
                </p>
              </div>
              <DataTable columns={combinedColumns} data={filteredAllOnboardings} emptyMessage="No onboarding data found."/>
            </div>)}

          {(filterType === 'all' || filterType === 'customer') && (<div className="p-6 rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
              <div className="flex items-center justify-between mb-4">
              ...
              {customerError && (<div className="mb-4 p-3 rounded-lg bg-[hsl(var(--destructive))]/10 border border-red-200 dark:border-red-800 text-[hsl(var(--destructive))] text-sm">
                  Error loading customer onboardings: {customerError instanceof Error ? customerError.message : 'Unknown error'}
                </div>)}
              <DataTable columns={customerColumns} data={filteredCustomerOnboardings} emptyMessage="No customer onboarding data found."/>
            </div>)}

          {(filterType === 'all' || filterType === 'vendor') && (<div className="p-6 rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold text-[hsl(var(--foreground))] flex items-center gap-2">
              ...
              {vendorError && (<div className="mb-4 p-3 rounded-lg bg-[hsl(var(--destructive))]/10 border border-red-200 dark:border-red-800 text-[hsl(var(--destructive))] text-sm">
                  Error loading vendor onboardings: {vendorError instanceof Error ? vendorError.message : 'Unknown error'}
                </div>)}
              <DataTable columns={vendorColumns} data={filteredVendorOnboardings} emptyMessage="No vendor onboarding data found."/>
            </div>)}
        </div>)}

      {/* Onboarding Details Modal */}
      {selectedOnboarding && (<OnboardingDetailsModal isOpen={isOnboardingModalOpen} onClose={handleCloseOnboardingModal} organizationId={selectedOnboarding.organizationId} organizationType={selectedOnboarding.organizationType} organizationName={selectedOnboarding.organizationName}/>)}
    </div>);
}
//# sourceMappingURL=OnboardingDataPage.js.map