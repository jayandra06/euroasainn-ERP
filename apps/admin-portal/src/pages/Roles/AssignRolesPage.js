...
export function AssignRolesPage() {
    ...
    const removeRoleMutation = useMutation({
        mutationFn: async (userId) => {
            await authFetch(`${API_URL}/assign-role/assign/${userId}`, { method: "DELETE" });
        },
    ...
    return (<div className="p-6 space-y-10">
      ...
      <div className="border rounded-lg p-5 bg-white shadow-sm max-w-md space-y-4">
        ...
        <button disabled={!selectedRole || !selectedUser} onClick={() => assignRoleMutation.mutate({
      ...
      <div className="flex items-center bg-white p-4 rounded-lg border shadow-sm">
        <div className="relative w-72">
          ...
          <input className="border pl-10 p-2 w-full rounded-lg" placeholder="Search users..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
        </div>
      </div>
      ...
      <div className="border rounded-lg p-5 bg-white shadow-sm">
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {filteredUsers.map((u) => (<tr key={u._id} className="hover:bg-gray-50">
                ...
                setEditModalOpen(true);
                    ...
                    <button className="text-red-600" onClick={() => removeRoleMutation.mutate(u._id)}>
                      <MdDelete size={18}/>
                    </button>
                  </div>
                </td>
