...
import { MdFilterList, MdCheckCircle, MdCancel, MdSearch, MdDownload, MdRefresh } from 'react-icons/md';
...
export function LicensesPage() {
        ...
        if (filterOnboarding !== 'all') {
            filtered = filtered.filter((org) => {
                if (filterOnboarding === 'completed')
                    return org.onboardingCompleted === true;
                if (filterOnboarding === 'pending')
    ...
    const { data: orgsData, isLoading } = useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                const orgsWithLicenses = data.data;
                // Apply filters
    ...
    const columns = [
        {
            key: 'name',
            header: 'Organization',
            render: (org) => (<div className="font-semibold text-[hsl(var(--foreground))] cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
          {org.name}
        </div>),
        },
        {
        ...
        },
        {
            key: 'onboarding',
            header: 'Onboarding',
            render: (org) => (<div className="flex items-center gap-2">
        ...
        },
        {
            ...
            render: (org) => {
                if (!org.license) {
                    return (<span className="px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full bg-gray-100 text-[hsl(var(--foreground))] font-semibold dark:bg-gray-900/50 ring-1 ring-[hsl(var(--border))]">
              No License
            </span>);
                }
                const statusColors = {
                ...
                return (<span className={cn('px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full ring-1', statusColors[org.license.status] || statusColors.active)}>
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 px-4 py-2 rounded-xl bg-[hsl(var(--secondary))] border border-[hsl(var(--border))] focus-within:border-emerald-500 dark:focus-within:border-emerald-400 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
            ...
            <input type="text" placeholder="Search organizations..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 bg-transparent border-none outline-none text-sm text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))]"/>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 font-medium">
              <option value="all">All Types</option>
              <option value="customer">Customer Only</option>
              <option value="vendor">Vendor Only</option>
            </select>
            <select value={filterOnboarding} onChange={(e) => setFilterOnboarding(e.target.value)} className="px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 font-medium">
              <option value="all">All Onboarding</option>
              <option value="completed">Completed</option>
              <option value="pending">Pending</option>
            </select>
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200 font-medium">
      ...
      {isLoading ? (<div className="p-12 text-center rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
          ...
          <DataTable columns={columns} data={filteredOrgs || []} onRowClick={handleRowClick} emptyMessage="No organizations found."/>
        </div>)}
    </div>);
}
//# sourceMappingURL=LicensesPage.js.map