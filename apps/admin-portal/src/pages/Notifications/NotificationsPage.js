...
export function NotificationsPage() {
    ...
    const filteredNotifications = notifications.filter((notif) => {
        if (filterType !== 'all' && notif.type !== filterType)
            return false;
        if (filterRead === 'read' && !notif.read)
            return false;
        if (filterRead === 'unread' && notif.read)
            return false;
        return true;
    });
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          {unreadCount > 0 && (<button onClick={handleMarkAllAsRead} className="flex items-center gap-2 px-4 py-2 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-white rounded-xl transition-colors font-semibold text-sm">
              <MdMarkEmailRead className="w-4 h-4"/>
              Mark All Read
            </button>)}
          <button onClick={handleSettings} className="flex items-center gap-2 px-4 py-2 bg-[hsl(var(--card))] border border-[hsl(var(--border))] hover:bg-[hsl(var(--muted))] rounded-xl transition-colors font-semibold text-sm text-[hsl(var(--foreground))]">
      ...
      <div className="p-6 rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <select value={filterRead} onChange={(e) => setFilterRead(e.target.value)} className="px-4 py-2 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] shadow-sm focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all duration-200 font-medium">
      ...
      <div className="space-y-4">
        {filteredNotifications.length === 0 ? (<div className="p-12 text-center rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
            ...
            <p className="text-lg font-semibold text-[hsl(var(--muted-foreground))]">No notifications found</p>
          </div>) : (filteredNotifications.map((notif) => (<div key={notif.id} className={cn('p-6 rounded-xl border shadow-sm transition-all', notif.read
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2 ml-4">
                  {!notif.read && (<button onClick={() => handleMarkAsRead(notif.id)} className="p-2 rounded-lg bg-indigo-100 dark:bg-indigo-900/50 text-[hsl(var(--foreground))] font-semibold hover:bg-indigo-200 dark:hover:bg-indigo-900 transition-colors" title="Mark as read">
                      <MdCheckCircle className="w-5 h-5"/>
                    </button>)}
                  <button onClick={() => handleDelete(notif.id)} className="p-2 rounded-lg bg-red-100 dark:bg-red-900/50 text-[hsl(var(--destructive))] hover:bg-red-200 dark:hover:bg-red-900 transition-colors" title="Delete">
