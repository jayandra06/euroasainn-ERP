...
export function CatalogManagementPage() {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (data.success) {
                showToast(data.message || `Successfully uploaded ${data.data?.created || 0} items`, 'success');
                // Clear selected file
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (createdItems.length > 0) {
                showToast(`Successfully saved ${createdItems.length} product(s)${errors.length > 0 ? ` (${errors.length} failed)` : ''}`, 'success');
    ...
    return (<div className="w-full min-h-screen p-8">
      ...
      <div className="mb-8">
        ...
        <div className="mb-4">
          <button onClick={handleUploadFileClick} className="w-full sm:w-auto px-6 py-3 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))] rounded-lg font-medium transition-colors">
            Upload File {selectedFile && `(${selectedFile.name})`}
          </button>
        </div>
        <div className="flex gap-3">
          <button onClick={handleDownloadTemplate} className="px-4 py-2 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))] rounded-lg font-medium transition-colors flex items-center gap-2">
            <MdDownload className="w-4 h-4"/>
            Download Template
          </button>
          <button onClick={handleUpload} disabled={!selectedFile} className="px-4 py-2 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))] rounded-lg font-medium transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            <MdUpload className="w-4 h-4"/>
            Upload
          </button>
          <button onClick={handleView} className="px-4 py-2 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))] rounded-lg font-medium transition-colors">
            View
          </button>
        </div>
      </div>
      ...
      <div>
              ...
              <tbody className="bg-[hsl(var(--card))] divide-y divide-gray-200 dark:divide-gray-800">
                {products.map((product) => (<tr key={product.id}>
                    ...
                    <td className="px-4 py-3">
                      <input type="text" value={product.impa} onChange={(e) => handleInputChange(product.id, 'impa', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.description} onChange={(e) => handleInputChange(product.id, 'description', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.partNo} onChange={(e) => handleInputChange(product.id, 'partNo', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.positionNo} onChange={(e) => handleInputChange(product.id, 'positionNo', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.alternativeNo} onChange={(e) => handleInputChange(product.id, 'alternativeNo', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.brand} onChange={(e) => handleInputChange(product.id, 'brand', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.model} onChange={(e) => handleInputChange(product.id, 'model', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.category} onChange={(e) => handleInputChange(product.id, 'category', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.dimensions} onChange={(e) => handleInputChange(product.id, 'dimensions', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.price} onChange={(e) => handleInputChange(product.id, 'price', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm" placeholder="0.00"/>
                    </td>
                    <td className="px-4 py-3">
                      <input type="text" value={product.remarks} onChange={(e) => handleInputChange(product.id, 'remarks', e.target.value)} className="w-full px-2 py-1 border border-[hsl(var(--border))] rounded bg-[hsl(var(--card))] text-[hsl(var(--foreground))] text-sm"/>
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => handleDeleteProduct(product.id)} className="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button onClick={handleSave} disabled={isSaving || products.length === 0} className="px-6 py-2 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))] rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              {isSaving ? 'Saving...' : 'Save'}
            </button>
            <button onClick={handleAddProduct} className="px-6 py-2 bg-gray-900 dark:bg-gray-800 hover:bg-gray-800 dark:hover:bg-gray-700 text-white rounded-lg font-medium transition-colors">
              Add Product
            </button>
          </div>
        </div>
...
//# sourceMappingURL=CatalogManagementPage.js.map