...
import { MdLocalShipping, MdCheckCircle, MdSchedule, MdWarning, MdInventory, } from 'react-icons/md';
// Mock data
const ordersData = [
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="p-6 rounded-xl border border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-950/20 shadow-sm">
        ...
        <div className="space-y-3">
          {pendingShipments.map((shipment, index) => (<div key={index} className="p-4 rounded-lg bg-[hsl(var(--card))] border border-orange-200 dark:border-orange-800">
              <div className="flex items-center justify-between mb-2">
                <div>
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-3 py-1 text-xs font-semibold rounded-full ${shipment.priority === 'High'
      ...
      <div className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
        ...
        <div className="space-y-4">
          {logisticsPartnerPerformance.map((partner, index) => (<div key={index} className="p-4 rounded-lg bg-[hsl(var(--secondary))]">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                <div className="h-2 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600" style={{ width: `${partner.onTime}%` }}/>
              </div>
            </div>))}
        </div>
      </div>
      ...
      <div className="p-6 rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/20 shadow-sm">
        ...
        <div className="space-y-3">
          {inventoryRestockPrediction.map((item, index) => {
            ...
            return (<div key={index} className="p-4 rounded-lg bg-[hsl(var(--card))] border border-blue-200 dark:border-blue-800">
                <div className="flex items-center justify-between mb-3">
                  <div>
                  ...
                  {needsReorder ? (<span className="px-3 py-1 text-xs font-semibold rounded-full bg-red-100 dark:bg-red-900 text-[hsl(var(--foreground))] font-semibold">
                      Reorder Now
                    </span>) : (<span className="px-3 py-1 text-xs font-semibold rounded-full bg-emerald-100 dark:bg-emerald-900 text-[hsl(var(--foreground))] font-semibold">
                      {item.daysUntilReorder} days
                    </span>)}
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div className={`h-2 rounded-full ${needsReorder ? 'bg-red-600' : stockPercent < 120 ? 'bg-orange-600' : 'bg-emerald-600'}`} style={{ width: `${Math.min(stockPercent, 100)}%` }}/>
                </div>
                {needsReorder && (<p className="text-xs text-red-600 mt-2">
      ...
      <div className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie data={orderStatusData} cx="50%" cy="50%" labelLine={false} outerRadius={100} fill="#8884d8" dataKey="count" label={({ status, count, percent }) => `${status}: ${count} (${(percent * 100).toFixed(0)}%)`}>
...
//# sourceMappingURL=SupplyChainDashboard.js.map