...
import { MdLocalShipping, MdAccountBalance, MdInventory, MdTrendingUp, MdDescription, MdRefresh, MdDownload, } from 'react-icons/md';
      ...
      <div className="border-b border-[hsl(var(--border))]">
        <nav className="flex space-x-8 overflow-x-auto" aria-label="Tabs">
          {analyticsTabs.map((tab) => {
            ...
            return (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn('flex items-center gap-2 py-4 px-1 border-b-2 font-medium text-sm whitespace-nowrap transition-colors', isActive
