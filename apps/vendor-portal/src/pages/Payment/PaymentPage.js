...
export function PaymentPage() {
    ...
    const plans = [
        {
            ...
            price: 79,
            period: 'month',
            features: ['Up to 10 users', 'Unlimited items', 'Basic support', 'Standard features'],
        },
        {
            name: 'Yearly Plan',
            price: 799,
            period: 'year',
            features: ['Up to 10 users', 'Unlimited items', 'Priority support', 'All features', '20% savings'],
            popular: true,
        },
    ];
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            default:
                return <MdInfo className="w-6 h-6 text-[hsl(var(--muted-foreground))]"/>;
        }
    };
    ...
    return (<div className="w-full min-h-screen p-8 space-y-6">
      ...
      {paymentStatus && (<div className={cn('p-6 rounded-2xl border-2 shadow-lg', paymentStatus.hasActivePayment
      ...
      {/* Subscription Plans */}
      {!paymentStatus?.hasActivePayment && (<div className="p-6 rounded-2xl bg-[hsl(var(--card))]/80 backdrop-blur-xl border border-[hsl(var(--border))]/50 shadow-lg">
          <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {plans.map((plan) => (<div key={plan.name} className={cn('p-6 rounded-xl border-2 transition-all', plan.popular
                ...
                {plan.popular && (<div className="inline-block px-3 py-1 mb-4 bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] text-xs font-bold rounded-full">
                    MOST POPULAR
                  </div>)}
                ...
                <ul className="space-y-2 mb-6">
                  {plan.features.map((feature, idx) => (<li key={idx} className="flex items-center gap-2 text-[hsl(var(--foreground))]">
                      <MdCheckCircle className="w-5 h-5 text-[hsl(var(--foreground))] font-semibold flex-shrink-0"/>
                      <span>{feature}</span>
                    </li>))}
                </ul>
                <button onClick={() => handlePayment(plan.price)} disabled={createPaymentMutation.isPending} className={cn('w-full py-3 px-4 rounded-lg font-semibold transition-colors', plan.popular
                    ? 'bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))]'
                    : 'bg-[hsl(var(--secondary))] hover:bg-[hsl(var(--muted))] text-[hsl(var(--foreground))]', createPaymentMutation.isPending && 'opacity-50 cursor-not-allowed')}>
        ...
        {payments && payments.length > 0 ? (<div className="overflow-x-auto">
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-[hsl(var(--border))]">
                {payments.map((payment) => (<tr key={payment._id} className="hover:bg-[hsl(var(--muted))]">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-[hsl(var(--muted-foreground))]">
                      {payment.subscriptionPeriod ? (<div className="text-sm">
                          <div>{new Date(payment.subscriptionPeriod.startDate).toLocaleDateString()}</div>
                          <div className="text-xs text-[hsl(var(--muted-foreground))]">to</div>
          ...
          </div>) : (<div className="p-16 text-center rounded-xl border border-[hsl(var(--border))]">
            <MdPayment className="w-16 h-16 text-[hsl(var(--muted-foreground))] mx-auto mb-4"/>
