...
export class PaymentController {
    ...
    async updatePaymentStatus(req, res) {
        try {
            ...
            const payment = await paymentService.updatePaymentStatus(id, status, transactionId, gatewayResponse);
            res.status(200).json({
                success: true,
                data: payment,
    ...
    async getPaymentsByOrganization(req, res) {
        try {
        ...
        try {
            ...
            if (organization) {
                ...
                if (isVendorOrg) {
                    ...
                    const isExternalVendor = invitedBy === 'customer' ||
                        hasInvitedByOrgId ||
            ...
            const paymentStatus = await paymentService.checkOrganizationPaymentStatus(user.organizationId.toString());
            res.status(200).json({
                success: true,
                data: {
    ...
    async handlePaymentWebhook(req, res) {
        try {
            ...
            if (event === 'payment.captured') {
                ...
                const payment = await import('../models/payment.model').then(m => m.Payment.findOne({ transactionId: orderId }));
                if (payment) {
                    // Verify payment signature
                    const { razorpayService } = await import('../services/razorpay.service');
                    if (razorpayService) {
                        const isValid = await razorpayService.verifyPaymentSignature(orderId, paymentId, paymentEntity.signature || '');
                        if (isValid) {
                            await paymentService.updatePaymentStatus(payment._id.toString(), PaymentStatus.SUCCESS, paymentId, paymentEntity);
                            ...
                            await paymentService.updatePaymentStatus(payment._id.toString(), PaymentStatus.FAILED, paymentId, paymentEntity);
                        }
                    }
                    else {
                ...
                const payment = await import('../models/payment.model').then(m => m.Payment.findOne({ transactionId: orderId }));
                if (payment) {
                    await paymentService.updatePaymentStatus(payment._id.toString(), PaymentStatus.FAILED, paymentEntity.id, paymentEntity);
    ...
    async verifyPayment(req, res) {
        try {
            ...
            if (isValid) {
                ...
                await paymentService.updatePaymentStatus(payment._id.toString(), PaymentStatus.SUCCESS, paymentId, paymentDetails);
                res.status(200).json({
                    success: true,
                    data: payment,
                ...
                await paymentService.updatePaymentStatus(payment._id.toString(), PaymentStatus.FAILED, paymentId, { error: 'Invalid signature' });
                res.status(400).json({
                    success: false,
