...
export class OrganizationController {
    async createOrganization(req, res) {
        try {
            ...
            if (existingVendor && existingVendor.isAdminInvited) {
                await organizationService.addCustomerToVendorVisibility(existingVendor._id.toString(), requester.organizationId);
            ...
            if (existingVendor && !existingVendor.isAdminInvited && requester?.organizationId) {
                ...
                const hasAccess = existingVendor.visibleToCustomerIds?.some((id) => id.toString() === customerOrgId.toString());
                if (hasAccess) {
                    return res.status(200).json({
                        success: true,
    ...
    async getOrganizations(req, res) {
        try {
            ...
            const organizations = await organizationService.getOrganizations(type, portalType, filters);
            res.status(200).json({
                success: true,
                data: organizations,
            });
    ...
    async resendOrganizationInvitation(req, res) {
        try {
    ...
    async revokeOrganizationInvitation(req, res) {
        try {
            const { id, invitationId } = req.params;
    ...
    async getOrganizationsWithLicenses(req, res) {
        try {
            ...
            customerOnboardings.forEach((onboarding) => {
            ...
            vendorOnboardings.forEach((onboarding) => {
                const orgId = onboarding.organizationId?.toString();
                if (orgId) {
            ...
            const orgsWithLicenses = organizations.map((org) => {
    ...
    async inviteOrganizationAdmin(req, res) {
        try {
            ...
            const responseUser = { ...invitedUser };
            if (emailSent) {
