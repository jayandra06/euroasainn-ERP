...
export class QuotationService {
    ...
    async createQuotation(organizationId, data) {
        ...
        const metadataTerms = data.metadata?.terms || {};
                ...
                if (rfq && rfq.organizationId) {
                    ...
                    if (customerUsers.length > 0) {
                        ...
                        await emailService.sendQuotationNotificationEmail({
                            ...
                            vendorOrganizationName: vendorOrg?.name || 'Vendor',
     ...
     */
    async getQuotationByRFQIdForVendor(rfqId, vendorOrganizationId) {
        const mongoose = await import('mongoose');
        ...
        return {
            ...
            items: quotation.metadata?.items || quotation.items?.map((item) => ({
    ...
    async getQuotationById(quotationId, organizationId) {
    ...
    async updateQuotation(quotationId, organizationId, data) {
        const quotation = await Quotation.findOne({ _id: quotationId, organizationId });
        if (!quotation) {
            throw new Error('Quotation not found');
        }
    ...
    async deleteQuotation(quotationId, organizationId) {
     ...
     */
    async createAdminQuotation(data) {
        const mongoose = await import('mongoose');
        // Transform items from frontend format to model format
        let transformedItems = [];
        if (data.items && Array.isArray(data.items)) {
     ...
     */
    async getQuotationByRFQIdForAdmin(rfqId) {
        const quotation = await Quotation.findOne({
            rfqId: rfqId,
            isAdminOffer: true,
        })
    ...
    async getQuotationsByRFQId(rfqId) {
        ...
        return quotations.map((q) => ({
            ...
            items: q.metadata?.items || q.items?.map((item) => ({
     ...
     */
    async finalizeOffer(quotationId, customerOrganizationId) {
        const { RFQ } = await import('../models/rfq.model');
        const { Organization } = await import('../models/organization.model');
        const { User } = await import('../models/user.model');
        const { logger } = await import('../config/logger');
            ...
            if (vendorOrganizationId) {
                ...
                if (vendorUsers.length > 0) {
                    ...
                    const totalAmount = quotation.totalAmount ||
                        (quotation.items?.reduce((sum, item) => sum + ((item.unitPrice || 0) * (item.quantity || 0)), 0) || 0);
