...
function calculatePayrollTotals(payroll) {
    ...
    const grossSalary = (payroll.base || 0) +
        (payroll.hra || 0) +
        (payroll.ta || 0) +
        (payroll.da || 0) +
        (payroll.sa || 0) +
    ...
    const totalDeductions = (payroll.pf || 0) +
        (payroll.tds || 0) +
        (payroll.insurance || 0) +
...
export class EmployeeService {
    ...
    async getEmployeesWithOnboardingStatus(organizationId, filters) {
        try {
            ...
            const onboardingQuery = { organizationId: new mongoose.Types.ObjectId(organizationId) };
    ...
    async updateEmployee(employeeId, organizationId, data) {
        const employee = await Employee.findOne({ _id: employeeId, organizationId });
        if (!employee) {
            throw new Error('Employee not found');
        }
        ...
        if (data.businessUnitId !== undefined) {
            if (data.businessUnitId && data.businessUnitId !== 'unassigned') {
                employee.businessUnitId = new mongoose.Types.ObjectId(data.businessUnitId);
            }
            else {
    ...
    async inviteEmployee(organizationId, data) {
    ...
    async completeEmployeeOnboarding(token, password) {
    ...
    async submitEmployeeOnboardingForm(token, data) {
        // Import InvitationToken model
        const { InvitationToken } = await import('../models/invitation-token.model');
        // Verify invitation token
        const invitation = await InvitationToken.findOne({
    ...
    async getEmployeeOnboardingByToken(token) {
        // Import InvitationToken model
        const { InvitationToken } = await import('../models/invitation-token.model');
        // Verify invitation token
        const invitation = await InvitationToken.findOne({
    ...
    async getEmployeeOnboardings(organizationId, filters) {
        const query = { organizationId: new mongoose.Types.ObjectId(organizationId) };
        if (filters?.status) {
    ...
    async getEmployeeOnboardingById(onboardingId, organizationId) {
    ...
    async approveEmployeeOnboarding(onboardingId, organizationId, approvedBy, _remarks) {
    ...
    async rejectEmployeeOnboarding(onboardingId, organizationId, rejectedBy, rejectionReason) {
        const onboarding = await EmployeeOnboarding.findOne({
            _id: onboardingId,
            organizationId: new mongoose.Types.ObjectId(organizationId),
        });
    ...
    async deleteEmployeeOnboarding(onboardingId, organizationId) {
        // Validate onboardingId format
        if (!mongoose.Types.ObjectId.isValid(onboardingId)) {
            throw new Error(`Invalid onboarding ID format: ${onboardingId}`);
        }
