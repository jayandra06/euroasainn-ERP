...
class AssignRoleService {
    async listUsers(portalType, organizationId) {
        const cacheKey = `assign-role:users:${organizationId}:${portalType}`;
        const cached = await redisService.getCacheJSON(cacheKey);
        if (cached)
            return cached;
    ...
    async listRoles(portalType, organizationId) {
    ...
    async assignRole(userId, roleId, organizationId) {
        ...
        const [user, role] = await Promise.all([
            User.findOne({ _id: userId, organizationId }).lean(),
            Role.findOne({ _id: roleId, organizationId }).lean(),
        ]);
        if (!user)
        ...
        const existingRoles = (await enforcer.getGroupingPolicy()).filter(r => r[0] === userIdStr && r[2] === organizationId);
        ...
        await enforcer.addNamedGroupingPolicy("g4", role.key, role.key, portal);
...
//# sourceMappingURL=assign-role.service.js.map