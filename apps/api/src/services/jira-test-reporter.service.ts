...
export class JiraTestReporterService {
   ...
   */
  async createIssueForTestFailure(failure: TestFailure): Promise<string | null> {
    try {
   ...
   */
  async createTestRunSummary(summary: TestRunSummary): Promise<string | null> {
    try {
      if (summary.failed === 0) {
...
`;
   ...
   */
  private buildFailureDescription(failure: TestFailure): string {
   ...
   */
  private async findExistingIssue(testName: string): Promise<string | null> {
    try {
   ...
   */
  async assignIssueToDeveloper(issueKey: string, filePath: string): Promise<void> {
    try {
   ...
   */
  private getAssigneeMap(): Record<string, string> {
    try {
