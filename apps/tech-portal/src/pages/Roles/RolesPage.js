      ...
      <div className="bg-white border rounded-lg p-6 space-y-4 shadow-sm">
        ...
        {portal && (<div className="grid grid-cols-2 gap-3">
            {permissionsList.map((perm) => (<label key={perm.key} className="flex gap-2 text-sm">
                <input type="checkbox" checked={permissions[perm.key] || false} onChange={() => setPermissions((prev) => ({
                    ...prev,
        ...
        <button className="bg-blue-600 text-white px-4 py-2 rounded-lg" disabled={!canCreate} onClick={handleAddRole}>
          + Add Role
        </button>
      </div>
          ...
          <tbody>
            {roles.map((r) => (<tr key={r._id}>
                ...
                <td className="border p-2 text-center">
                  <button disabled={!canUpdate} onClick={() => {
                if (!canUpdate)
                    return;
                ...
                setIsEditModalOpen(true);
                  ...
                  <button disabled={!canDelete} onClick={() => canDelete && deleteRole(r._id)} className="hover:bg-red-100 p-1 rounded ml-2">
                    <MdDelete size={18}/>
                  </button>
                </td>
              </tr>))}
      ...
      {isEditModalOpen && editingRole && (<div className="fixed inset-0 bg-black/30 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg w-[400px] relative">
            ...
            <div className="grid grid-cols-2 gap-3">
              {permissionsList.map((perm) => (<label key={perm.key} className="flex gap-2 text-sm">
                  <input type="checkbox" checked={editingRole.permissions[perm.key] || false} onChange={() => setEditingRole({
                    ...editingRole,
                    permissions: {
            ...
            <button className="bg-blue-600 text-white py-2 rounded-lg w-full mt-4" disabled={!canUpdate} onClick={saveEdit}>
              Save Changes
            </button>
          </div>
        </div>)}
