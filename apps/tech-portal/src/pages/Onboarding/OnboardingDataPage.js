...
export function OnboardingDataPage() {
    ...
    const rejectCustomerMutation = useMutation({
        mutationFn: async ({ id, reason }) => {
            ...
            if (!response.ok)
                throw new Error('Failed to reject onboarding');
            return response.json();
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['customer-onboardings'] });
    ...
    const { data: onboardingDetails } = useQuery({
        ...
        queryFn: async () => {
            if (!selectedOnboarding || !selectedType)
                return null;
    ...
    const customerColumns = [
        {
            key: 'companyName',
            header: 'Company Name',
            render: (item) => (<button onClick={() => handleViewDetails(item, 'customer')} className="font-semibold text-[hsl(var(--foreground))] font-semibold hover:underline text-left">
          {item.companyName}
        </button>),
        },
        {
            ...
            render: (item) => (<span className="text-gray-600 dark:text-gray-400">{item.contactPerson}</span>),
        },
        {
            key: 'email',
            header: 'Email',
        ...
        },
        {
            ...
            render: (item) => {
                ...
                if (isApproved) {
                    return (<span className="inline-flex items-center" title="Approved">
              <MdCheckCircle className="w-6 h-6 text-[hsl(var(--foreground))] font-semibold"/>
            </span>);
                }
                if (isUnderReview) {
                    return (<span className="inline-flex items-center" title={item.status === 'completed' ? 'Under Review' : 'Pending'}>
              <MdInfo className="w-6 h-6 text-amber-600 dark:text-amber-400"/>
            </span>);
                }
                if (item.status === 'rejected') {
                    return (<span className="inline-flex items-center" title="Rejected">
        ...
        },
        {
            key: 'submittedAt',
            header: 'Submitted At',
            render: (item) => (<span className="text-gray-600 dark:text-gray-400">
        ...
        },
        {
          ...
          {item.status === 'completed' && (<>
              <button onClick={() => {
                    ...
                    }} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-1" title="Approve">
              ...
              <button onClick={() => {
                    ...
                    }} className="px-3 py-1.5 bg-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive))]/90 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-1" title="Reject">
                <MdCancel className="w-6 h-6"/>
                Reject
              </button>
            </>)}
          {item.status === 'approved' && (<span className="text-sm font-medium text-[hsl(var(--foreground))] font-semibold">
              Approved
            </span>)}
          {item.status === 'rejected' && (<span className="text-sm font-medium text-red-600 dark:text-red-400">
              Rejected
            </span>)}
        </div>),
        },
    ...
    const vendorColumns = [
        {
            key: 'companyName',
            header: 'Company Name',
            render: (item) => (<button onClick={() => handleViewDetails(item, 'vendor')} className="font-semibold text-[hsl(var(--foreground))] font-semibold hover:underline text-left">
          {item.companyName}
        </button>),
        },
        {
        ...
        },
        {
          ...
          {item.status === 'approved' && (<span className="px-3 py-1.5 inline-flex items-center gap-1.5 text-sm font-medium rounded-lg bg-emerald-100 text-[hsl(var(--foreground))] font-semibold dark:bg-emerald-900/50">
              <MdCheckCircle className="w-4 h-4"/>
              Approved
            </span>)}
          {item.status === 'rejected' && (<span className="px-3 py-1.5 inline-flex items-center gap-1.5 text-sm font-medium rounded-lg bg-red-100 text-[hsl(var(--foreground))] font-semibold dark:bg-red-900/50">
              <MdCancel className="w-6 h-6"/>
              Rejected
            </span>)}
        </div>),
    ...
    const combinedColumns = [
        ...
        },
        {
            key: 'companyName',
            header: 'Company Name',
            render: (item) => (<button onClick={() => handleViewDetails(item, item.type)} className="font-semibold text-[hsl(var(--foreground))] font-semibold hover:underline text-left">
          {item.companyName}
        </button>),
        },
        {
    ...
    return (<div className="w-full space-y-6">
      ...
      {/* Tables */}
      {isLoading ? (<div className="p-12 text-center rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
        ...
        </div>) : (customerError || vendorError) ? (<div className="p-12 text-center rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/20 shadow-sm">
          ...
          {filterType === 'all' && (<div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
              <div className="p-6">
                <DataTable columns={combinedColumns} data={allOnboardings} emptyMessage="No onboarding data found."/>
              </div>
            </div>)}

          {(filterType === 'all' || filterType === 'customer') && (<div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
              <div className="p-6">
                ...
                <DataTable columns={customerColumns} data={customerOnboardings || []} emptyMessage="No customer onboarding data found."/>
              </div>
            </div>)}

          {(filterType === 'all' || filterType === 'vendor') && (<div className="rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
              <div className="p-6">
                <h2 className="text-xl font-semibold text-[hsl(var(--foreground))] mb-4 flex items-center gap-2">
                  <MdPerson className="w-6 h-6 text-[hsl(var(--foreground))] font-semibold"/>
                  Vendor Onboardings
                </h2>
                <DataTable columns={vendorColumns} data={vendorOnboardings || []} emptyMessage="No vendor onboarding data found."/>
              </div>
            </div>)}
        </div>)}
      ...
      <Modal isOpen={isModalOpen} onClose={() => {
            ...
            setSelectedOnboarding(null);
            setSelectedType(null);
        }} title={`${selectedType === 'customer' ? 'Customer' : 'Vendor'} Onboarding Details - ${selectedOnboarding?.companyName || ''}`} size="large">
        {onboardingDetails ? (<div className="space-y-6">
            {/* Basic Information */}
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-[hsl(var(--foreground))]">
                    <span className={cn('px-2 py-1 rounded text-xs font-semibold', onboardingDetails.status === 'approved' ? 'bg-emerald-100 text-[hsl(var(--foreground))] font-semibold dark:bg-emerald-900/50' :
            ...
            {selectedType === 'vendor' && (<>
                {onboardingDetails.managingDirector && (<div>
                    <h3 className="text-lg font-semibold text-[hsl(var(--foreground))] mb-4 pb-2 border-b border-[hsl(var(--border))]">
                      Managing Director
                    </h3>
                    <div className="grid grid-cols-2 gap-4">
                ...
                {onboardingDetails.salesManager && (<div>
                    <h3 className="text-lg font-semibold text-[hsl(var(--foreground))] mb-4 pb-2 border-b border-[hsl(var(--border))]">
                      Sales Manager
                    </h3>
                    <div className="grid grid-cols-2 gap-4">
                ...
                {(onboardingDetails.brands?.length > 0 || onboardingDetails.categories?.length > 0 || onboardingDetails.models?.length > 0) && (<div>
                    <h3 className="text-lg font-semibold text-[hsl(var(--foreground))] mb-4 pb-2 border-b border-[hsl(var(--border))]">
                      Product Information
                    </h3>
                    <div className="grid grid-cols-3 gap-4">
                      {onboardingDetails.brands?.length > 0 && (<div>
                          <label className="text-sm font-medium text-[hsl(var(--muted-foreground))]">Brands</label>
                          <div className="flex flex-wrap gap-2 mt-1">
                            {onboardingDetails.brands.map((brand, i) => (<span key={i} className="px-2 py-1 bg-blue-100 dark:bg-blue-900/50 text-[hsl(var(--foreground))] font-semibold rounded text-xs">
                                {brand}
                              </span>))}
                          </div>
                        </div>)}
                      {onboardingDetails.categories?.length > 0 && (<div>
                          <label className="text-sm font-medium text-[hsl(var(--muted-foreground))]">Categories</label>
                          <div className="flex flex-wrap gap-2 mt-1">
                            {onboardingDetails.categories.map((cat, i) => (<span key={i} className="px-2 py-1 bg-purple-100 dark:bg-purple-900/50 text-[hsl(var(--foreground))] font-semibold rounded text-xs">
                                {cat}
                              </span>))}
                          </div>
                        </div>)}
                      {onboardingDetails.models?.length > 0 && (<div>
                          <label className="text-sm font-medium text-[hsl(var(--muted-foreground))]">Models</label>
                          <div className="flex flex-wrap gap-2 mt-1">
                            {onboardingDetails.models.map((model, i) => (<span key={i} className="px-2 py-1 bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300 rounded text-xs">
                                {model}
                              </span>))}
                          </div>
                        </div>)}
            ...
            {selectedType === 'customer' && onboardingDetails.vessels && (<div>
                <h3 className="text-lg font-semibold text-[hsl(var(--foreground))] mb-4 pb-2 border-b border-[hsl(var(--border))]">
                  Vessel Information
                </h3>
                <div>
            ...
            {/* Action Buttons */}
            {onboardingDetails.status === 'completed' && (<div className="flex items-center justify-end gap-4 pt-4 border-t border-[hsl(var(--border))]">
                <button onClick={handleReject} disabled={rejectCustomerMutation.isPending || rejectVendorMutation.isPending} className="px-4 py-2 bg-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive))]/90 disabled:opacity-50 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
                  <MdCancel className="w-5 h-5"/>
                  Reject
                </button>
                <button onClick={handleApprove} disabled={approveCustomerMutation.isPending || approveVendorMutation.isPending} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
                  <MdCheckCircle className="w-6 h-6"/>
                  Approve
                </button>
              </div>)}
