...
import { MdTrendingUp, MdAttachMoney, MdPeople, MdBusinessCenter, MdSpeed, MdVpnKey, MdAdminPanelSettings, MdSettings, } from 'react-icons/md';
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((stat, index) => {
            ...
            return (<div key={index} className={cn('relative p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm hover:shadow-md transition-shadow', stat.bgColor)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={250}>
            ...
            borderRadius: '8px',
              ...
              <Area type="monotone" dataKey="users" stroke="#3b82f6" fill="url(#userGradient)" strokeWidth={2}/>
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie data={orgDistributionData} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
                {orgDistributionData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
              </Pie>
              <Tooltip contentStyle={{
            background: 'white',
            border: '1px solid #e5e7eb',
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {quickAccessCards.map((card, index) => {
            ...
            return (<div key={index} onClick={() => navigate(card.path)} className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm hover:shadow-md hover:border-blue-300 dark:hover:border-blue-700 transition-all cursor-pointer group">
