...
import { MdBugReport, MdCheckCircle, MdSchedule, MdWarning, MdTrendingUp, MdPerson, } from 'react-icons/md';
// Mock data
    ...
    return (<div className="w-full space-y-6">
      ...
      <div className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie data={issueStatusData} cx="50%" cy="50%" labelLine={false} outerRadius={100} fill="#8884d8" dataKey="count" label={({ status, count, percent }) => `${status}: ${count} (${(percent * 100).toFixed(0)}%)`}>
              {issueStatusData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
            </Pie>
            <Tooltip />
          </PieChart>
        </ResponsiveContainer>
      ...
      <div className="p-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm">
        ...
        <div className="space-y-3">
          {developerPerformance.map((dev, index) => (<div key={index} className="p-4 rounded-lg bg-[hsl(var(--secondary))]">
              ...
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                <div className="h-2 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600" style={{ width: `${(dev.issuesClosed / 52) * 100}%` }}/>
              </div>
            </div>))}
        </div>
      </div>
...
//# sourceMappingURL=BugIssueTracking.js.map