...
import { MdEmail, MdPerson, MdWork, MdSecurity, MdCheckCircle, MdCancel, MdSave, MdAddCircleOutline, MdBusiness, } from 'react-icons/md';
...
const formatRoleName = (roleKey) => roleKey
    .split(/[_\s]+/)
    .filter(Boolean)
...
const ADMIN_PERMISSION_GROUPS = [
    ...
    },
    {
        key: 'organizations',
        label: 'Organizations',
        description: 'Manage customer and vendor organizations',
    },
    {
        key: 'onboarding',
        label: 'Onboarding',
        description: 'Review and approve customer/vendor onboarding submissions',
    },
    {
        key: 'licenses',
        label: 'Licenses',
        description: 'Create and manage organization licenses with pricing',
    },
    {
        key: 'invoices',
        label: 'Invoices',
        description: 'Generate and track invoices for organizations',
    },
    {
        key: 'transactions',
        label: 'Transactions',
...
const VENDOR_PERMISSION_GROUPS = [
...
const CUSTOMER_PERMISSION_GROUPS = [
...
const buildInitialPermissionSelection = (groups) => groups.reduce((acc, group) => {
    const actionKeys = group.actions ?? PERMISSION_ACTIONS.map((action) => action.key);
    const actions = actionKeys.reduce((map, actionKey) => ({
        ...map,
        [actionKey]: false,
    }), {});
    return {
...
const selectionToPermissionList = (selection, groups) => {
...
export function UserForm({ user, onSuccess, onCancel }) {
    ...
    const permissionGroups = useMemo(() => PERMISSION_GROUPS[formData.portalType] || [], [formData.portalType]);
    const [permissionSelection, setPermissionSelection] = useState(() => buildInitialPermissionSelection(permissionGroups));
    useEffect(() => {
        if (user) {
            setFormData({
                email: user.email || '',
    ...
    const roleOptions = useMemo(() => {
        let options = !rolesResponse || rolesResponse.length === 0
    ...
    const computedPermissionList = useMemo(() => selectionToPermissionList(permissionSelection, permissionGroups), [permissionSelection, permissionGroups]);
    const selectedModuleCount = useMemo(() => Object.values(permissionSelection).filter((value) => value.enabled).length, [permissionSelection]);
    useEffect(() => {
        ...
        if (!hasInitialisedRole) {
            if (user) {
                const existingRole = roleOptions.find((role) => typeof user.roleId === 'object'
                    ? role.mongoId === user.roleId?._id
                    : typeof user.roleId === 'string'
                        ? role.mongoId === user.roleId
                        : role.key === user.role) || roleOptions.find((role) => role.key === user.role);
                if (existingRole) {
                    setFormData((prev) => ({
                        ...prev,
    ...
    const handleToggleGroup = (groupKey) => {
        setPermissionSelection((prev) => {
            ...
            const updatedActions = Object.keys(current.actions).reduce((map, actionKey) => ({
                ...map,
                [actionKey]: nextEnabled ? (actionKey === 'read' ? true : current.actions[actionKey]) : false,
            }), {});
            return {
                ...prev,
                [groupKey]: {
    ...
    const handleToggleAction = (groupKey, actionKey) => {
        setPermissionSelection((prev) => {
            const current = prev[groupKey];
    ...
    return (<form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        }} disabled={!!user} className={cn('w-full px-4 py-3 rounded-xl border bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all disabled:bg-gray-100 dark:disabled:bg-gray-900 disabled:cursor-not-allowed', errors.email ? 'border-red-300 dark:border-red-700' : 'border-[hsl(var(--border))]')} placeholder="user@example.com"/>
        {errors.email && (<p className="mt-1 text-xs text-[hsl(var(--foreground))] font-semibold">{errors.email}</p>)}
      </div>
      ...
      <div>
        ...
        <input id="fullName" type="text" value={formData.fullName} onChange={(e) => {
            setFormData({ ...formData, fullName: e.target.value });
            setErrors({ ...errors, fullName: '' });
        }} className={cn('w-full px-4 py-3 rounded-xl border bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all', errors.fullName ? 'border-red-300 dark:border-red-700' : 'border-[hsl(var(--border))]')} placeholder="Jane Doe"/>
        {errors.fullName && (<p className="mt-1 text-xs text-[hsl(var(--foreground))] font-semibold">{errors.fullName}</p>)}
      </div>
      ...
      <div>
        ...
        <select id="portalType" value={formData.portalType} onChange={(e) => {
        ...
        }} disabled={!!user} className="w-full px-4 py-3 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all disabled:bg-gray-100 dark:disabled:bg-gray-900 disabled:cursor-not-allowed">
          {PORTAL_OPTIONS.map((option) => (<option key={option.value} value={option.value}>
              {option.label}
            </option>))}
        </select>
      </div>
      ...
      <div>
        ...
        <select id="role" value={selectedRoleValue} onChange={(e) => handleRoleSelect(e.target.value)} disabled={isRolesLoading} className={cn('w-full px-4 py-3 rounded-xl border bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all', errors.role ? 'border-red-300 dark:border-red-700' : 'border-[hsl(var(--border))]')}>
          {roleOptions.map((role) => (<option key={role.id} value={role.id}>
              {role.name}
            </option>))}
          <option value="__create__">Create a new role…</option>
        </select>
        {errors.role && (<p className="mt-1 text-xs text-[hsl(var(--foreground))] font-semibold">{errors.role}</p>)}
      </div>

      {/* Organization - Only for Customer/Vendor users */}
      {needsOrganization && (<div>
          ...
          <select id="organizationId" value={formData.organizationId} onChange={(e) => {
                setFormData({ ...formData, organizationId: e.target.value });
                setErrors({ ...errors, organizationId: '' });
            }} disabled={isOrganizationsLoading || !!user} className={cn('w-full px-4 py-3 rounded-xl border bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-[hsl(var(--primary))] transition-all', errors.organizationId ? 'border-red-300 dark:border-red-700' : 'border-[hsl(var(--border))]', (isOrganizationsLoading || !!user) && 'bg-gray-100 dark:bg-gray-900 cursor-not-allowed')}>
            <option value="">Select {formData.portalType === 'customer' ? 'customer' : 'vendor'} organization...</option>
            {organizationsData?.map((org) => (<option key={org._id} value={org._id}>
                {org.name}
              </option>))}
          </select>
          {errors.organizationId && (<p className="mt-1 text-xs text-[hsl(var(--foreground))] font-semibold">{errors.organizationId}</p>)}
          {isOrganizationsLoading && (<p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">Loading organizations...</p>)}
          {!isOrganizationsLoading && organizationsData && organizationsData.length === 0 && (<p className="mt-1 text-xs text-[hsl(var(--foreground))] font-semibold">
              No {formData.portalType === 'customer' ? 'customer' : 'vendor'} organizations found. Please create one first.
            </p>)}
        </div>)}

      {/* Active Status */}
      {user && (<div className="flex items-center gap-3 p-4 rounded-xl bg-[hsl(var(--secondary))] border border-[hsl(var(--border))]">
          <input type="checkbox" id="isActive" checked={formData.isActive} onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })} className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-[hsl(var(--primary))]"/>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-[hsl(var(--border))]">
        <button type="button" onClick={onCancel} className="flex items-center gap-2 px-6 py-3 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] font-semibold hover:bg-[hsl(var(--muted))] transition-colors" disabled={isSaving}>
          <MdCancel className="w-4 h-4"/>
          <span>Cancel</span>
        </button>
        <button type="submit" disabled={isSaving} className="px-6 py-3 rounded-xl bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm flex items-center gap-2">
          <MdSave className="w-4 h-4"/>
          {isSaving ? 'Saving...' : user ? 'Update User' : 'Send Invitation'}
        </button>
      </div>

      <Modal isOpen={isCreateRoleOpen} onClose={() => {
            if (!isCreatingRole) {
                setIsCreateRoleOpen(false);
            }
        }} title="Create Role" size="large">
        <div className="space-y-6">
          <div>
            <label htmlFor="roleName" className="block text-sm font-semibold text-[hsl(var(--foreground))] mb-2">
              Role Name <span className="text-red-500">*</span>
            </label>
            <input id="roleName" type="text" value={newRoleName} onChange={(e) => setNewRoleName(e.target.value)} className="w-full px-4 py-2.5 border border-[hsl(var(--border))] rounded-lg bg-[hsl(var(--card))] text-[hsl(var(--foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))] focus:border-transparent transition-all" placeholder="e.g. Deployment Manager"/>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-[hsl(var(--border))] overflow-hidden">
              <div className="hidden sm:grid grid-cols-[1fr_120px_120px_120px] bg-gray-50 dark:bg-gray-900/40 text-xs font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wide">
                ...
                {PERMISSION_ACTIONS.map((action) => (<div key={action.key} className="px-5 py-3 text-center">
                    {action.label}
                  </div>))}
              </div>
            ...
            const actionKeys = group.actions ?? PERMISSION_ACTIONS.map((action) => action.key);
            return (<div key={group.key} className="grid grid-cols-1 sm:grid-cols-[1fr_120px_120px_120px] items-start sm:items-center gap-4 px-4 sm:px-5 py-4 bg-[hsl(var(--card))]">
                      <div className="flex items-start sm:items-center gap-3">
                        <input id={`module-${group.key}`} type="checkbox" className="mt-1 sm:mt-0 w-4 h-4 text-[hsl(var(--primary))] border-[hsl(var(--border))] rounded focus:ring-[hsl(var(--primary))]" checked={selection?.enabled ?? false} onChange={() => handleToggleGroup(group.key)}/>
                        <div>
                          <label htmlFor={`module-${group.key}`} className="text-sm font-semibold text-[hsl(var(--foreground))]">
                            {group.label}
                          </label>
                          {group.description && (<p className="text-xs text-[hsl(var(--muted-foreground))] mt-0.5">{group.description}</p>)}
                        </div>
                      </div>
                      {PERMISSION_ACTIONS.map((action) => {
                    if (!actionKeys.includes(action.key)) {
                        return (<div key={`${group.key}-${action.key}`} className="hidden sm:flex items-center justify-center text-xs text-gray-300">
                              —
                            </div>);
                    }
                    const isEnabled = selection?.enabled ?? false;
                    const isActive = Boolean(selection?.actions?.[action.key]);
                    return (<div key={`${group.key}-${action.key}`} className="flex sm:block items-center justify-between sm:justify-center text-xs font-semibold text-gray-500 sm:text-sm">
                            <span className="sm:hidden text-[hsl(var(--muted-foreground))]">{action.label}</span>
                            <button type="button" onClick={() => handleToggleAction(group.key, action.key)} disabled={!isEnabled} className={cn('mt-1 inline-flex items-center justify-center rounded-full px-4 py-1.5 text-xs sm:text-sm font-semibold border transition-all duration-150', !isEnabled &&
                            'cursor-not-allowed bg-gray-100 dark:bg-gray-800 text-gray-400 border-[hsl(var(--border))]', isEnabled &&
                            !isActive &&
                            'bg-[hsl(var(--card))] text-gray-600 dark:text-gray-300 border-[hsl(var(--border))] hover:bg-[hsl(var(--muted))]', isEnabled && isActive && 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] border-blue-600 shadow-sm hover:bg-blue-700')}>
                              {isActive ? 'On' : 'Off'}
                            </button>
                          </div>);
                })}
          ...
          {createRoleError && (<div className="p-3 rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm">
              {createRoleError}
            </div>)}
        ...
        }} className="flex items-center gap-2 px-4 py-2 text-[hsl(var(--foreground))] bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg font-semibold transition-colors" disabled={isCreatingRole}>
              <MdCancel className="w-4 h-4"/>
              <span>Cancel</span>
            </button>
            <button type="button" onClick={handleCreateRole} disabled={isCreatingRole} className="flex items-center gap-2 px-4 py-2 bg-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/90 text-[hsl(var(--primary-foreground))] rounded-lg font-semibold transition-colors disabled:opacity-50 shadow-lg hover:shadow-xl">
