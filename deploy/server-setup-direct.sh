...
cat > $NGINX_DIR/$DOMAIN <<'NGINX_EOF'
server {
    listen 80;
    server_name erp.euroasianngroup.com www.erp.euroasianngroup.com;
...
# Setup SSL with Let's Encrypt (may fail if domain not pointing yet, that's OK)
echo "🔒 Setting up SSL certificate..."
certbot --nginx -d $DOMAIN -d www.$DOMAIN --non-interactive --agree-tos --email admin@euroasianngroup.com --redirect || echo "⚠️ SSL setup skipped (domain may not be pointing to this server yet)"
...
pm2 startup systemd -u $USER --hp /home/$USER || echo "⚠️ PM2 startup already configured"
